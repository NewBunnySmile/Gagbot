...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName("command")
        .setDescription("What eval to attempt to run...")
        .setRequired(true),
    )
    ...
    try {
      if (interaction.user.id !== interaction.client?.application?.owner?.id) {
        await interaction.reply(
          `You're not ${interaction.client?.application?.owner?.displayName}. Go away.`,
        );
        return;
      }
      ...
      if (res) {
        await interaction.reply({
          content: `Eval result: ${res}`,
          flags: MessageFlags.Ephemeral,
        });
      } else {
        await interaction.reply({
          content: `Command run. No return value.`,
          flags: MessageFlags.Ephemeral,
        });
      }
    } catch (err) {
