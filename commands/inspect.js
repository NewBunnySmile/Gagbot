...
const {
  getPronouns,
  getPronounsSet,
} = require("./../functions/pronounfunctions.js");
const {
  getWearable,
  getWearableName,
  getLockedWearable,
  deleteWearable,
  removeLockedWearable,
} = require("../functions/wearablefunctions.js");
const { canAccessChastityBra } = require("../functions/vibefunctions.js");
const { getChastityBra } = require("../functions/vibefunctions.js");
const {
  getChastityBraName,
  getChastityBraTimelock,
} = require("../functions/vibefunctions.js");
...
module.exports = {
  ...
  async execute(interaction) {
    try {
      let inspectuser = interaction.options.getUser("user")
      ...
      if (inspectuser != interaction.user && !headwearrestrictions.canInspect) {
        interaction.reply({
          content: `${titletext}You are blinded and cannot look at ${inspectuser}.`,
      ...
      if (getGag(inspectuser.id)) {
        ...
        inspectparts.push(
          `<:Gag:1073495437635506216> Gag: Not currently worn.`,
        );
      }
      // Headwear parts!
      ...
      if (getMitten(inspectuser.id)) {
        if (getMittenName(inspectuser.id)) {
          inspectparts.push(
            `<:mittens:1452425463757803783> Mittens: **${getMittenName(inspectuser.id)}**`,
          );
        } else {
        ...
        inspectparts.push(
          `<:mittens:1452425463757803783> Mittens: Not currently worn.`,
        );
      }
      // Wearable status
      ...
      if (getVibe(inspectuser.id)) {
        inspectparts.push(
          `<:MagicWand:1073504682540011520> Vibrators/toys: **${getVibe(
            inspectuser.id,
          )
        ...
        inspectparts.push(
          `<:MagicWand:1073504682540011520> Vibrator: Not currently worn.`,
        );
      }
      // Arousal status
      ...
      if (arousal)
        arousalblock = `Arousal: **${getArousalDescription(inspectuser.id)}**`;
      ...
      if (getChastity(inspectuser.id)) {
        let isLocked = canAccessChastity(
          inspectuser.id,
          interaction.user.id,
        ).access;
        ...
        let currentchastitybelt = getChastityName(inspectuser.id)
          ? getChastityName(inspectuser.id)
        ...
        if (!headwearrestrictions.canInspect) {
          ...
          inspectparts.push(
            `<:Chastity:1073495208861380629> Chastity Belt: **${currentchastitybelt}**\n-# ‎   ⤷ ${lockemoji} **Locked (blind)**`,
          );
        ...
        inspectparts.push(
          `<:Chastity:1073495208861380629> Chastity Belt: Not currently worn.`,
        );
      }
      // Chastity Bra status.
      // You'll be able to tell that it's locked, but nothing more.
      if (getChastityBra(inspectuser.id)) {
        let isLocked = canAccessChastityBra(
          inspectuser.id,
          interaction.user.id,
        ).access;
        let lockemoji = isLocked ? "🔑" : "🔒";
        ...
        let currentchastitybelt = getChastityBraName(inspectuser.id)
          ? getChastityBraName(inspectuser.id)
          : "Locked Up Nice and Tight!";
        let timelockedtext = "Timelocked (Open)";
        if (chastitykeyaccess == 1) {
          timelockedtext = "Timelocked (Keyed)";
        ...
        if (!headwearrestrictions.canInspect) {
          // Wearer is blind - they can only tell its on and locked. Nothing more.
          inspectparts.push(
            `<:chastitybra:1457992137164718152> Chastity Bra: **${currentchastitybelt}**\n-# ‎   ⤷ ${lockemoji} **Locked (blind)**`,
          );
        ...
        inspectparts.push(
          `<:chastitybra:1457992137164718152> Chastity Bra: Not currently worn.`,
        );
      }
      // Corset status
      ...
      if (getCorset(inspectuser.id)) {
        if (getCorset(inspectuser.id).tightness > 10) {
          inspectparts.push(
            `<:corset:1451126998192881684> Corset: **Laced beyond reason to a string length of ${getCorset(inspectuser.id).tightness}**`,
          );
        } else if (getCorset(inspectuser.id).tightness > 7) {
          inspectparts.push(
            `<:corset:1451126998192881684> Corset: **Laced tightly to a string length of ${getCorset(inspectuser.id).tightness}**`,
          );
        } else if (getCorset(inspectuser.id).tightness > 4) {
          inspectparts.push(
            `<:corset:1451126998192881684> Corset: **Laced moderately to a string length of ${getCorset(inspectuser.id).tightness}**`,
          );
        } else {
          inspectparts.push(
            `<:corset:1451126998192881684> Corset: **Laced loosely to a string length of ${getCorset(inspectuser.id).tightness}**`,
          );
        }
      } else {
        inspectparts.push(
          `<:corset:1451126998192881684> Corset: Not currently worn.`,
        );
      }
      ...
      if (getHeavy(inspectuser.id)) {
        inspectparts.push(
          `<:Armbinder:1073495590656286760> Heavy Bondage: **${getHeavy(inspectuser.id).type}**`,
        );
      } else {
        inspectparts.push(
          `<:Armbinder:1073495590656286760> Heavy Bondage: Not currently worn.`,
        );
      }
      // Collar status
      // You'll be able to tell that it's locked, but nothing more.
      ...
      if (getCollar(inspectuser.id)) {
        let currentcollartext = getCollarName(inspectuser.id)
          ? getCollarName(inspectuser.id)
          : "Locked Up Nice and Tight!";
        let isLocked = canAccessCollar(
          inspectuser.id,
          interaction.user.id,
        ).access;
        let lockemoji = isLocked ? "🔑" : "🔒";
        ...
        if (!headwearrestrictions.canInspect) {
          // Wearer is blind - they can only tell its on and locked. Nothing more.
          collarparts.push(
            `<:collar:1449984183261986939> Collar: **${currentcollartext}**\n-# ‎   ⤷ ${lockemoji} **Locked (blind)**`,
          );
        ...
        // Output Collar Perms
        collarparts.push(
          `-# Mittens: ${getCollarPerm(inspectuser.id, "mitten") ? "YES" : "NO"}, Chastity: ${getCollarPerm(inspectuser.id, "chastity") ? "YES" : "NO"}, Heavy: ${getCollarPerm(inspectuser.id, "heavy") ? "YES" : "NO"}, Masks: ${getCollarPerm(inspectuser.id, "mask") ? "YES" : "NO"}`,
        );
      } else {
        collarparts.push(
          `<:collar:1449984183261986939> Collar: Not currently worn.`,
        );
      }
      ...
      if (keysheldclonedchastity.length > 0) {
        keysheldclonedchastity = keysheldclonedchastity.map(
          (k) => `<@${k.split("_")[0]}>`,
        );
        let keysstring = keysheldclonedchastity.join(", ");
        keysheldtext = `${keysheldtext}- Cloned chastity belt keys: ${keysstring}\n`;
      }
      let keysheldclonedchastitybra = getClonedChastityBraKeysOwned(
        inspectuser.id,
      );
      if (keysheldclonedchastitybra.length > 0) {
        keysheldclonedchastitybra = keysheldclonedchastitybra.map(
          (k) => `<@${k.split("_")[0]}>`,
        );
      ...
      if (keysheldclonedcollar.length > 0) {
        keysheldclonedcollar = keysheldclonedcollar.map(
          (k) => `<@${k.split("_")[0]}>`,
        );
      ...
      console.log(
        `Inspect text generated was ${outtext.length} characters long.`,
      );
    } catch (err) {
      console.log(err);
    }
