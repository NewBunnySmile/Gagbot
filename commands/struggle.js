...
module.exports = {
  ...
  async autoComplete(interaction) {
    ...
    try {
      ...
      if (heavybondage) {
        outopts.push({
          name: `Heavy Bondage: ${getHeavy(interaction.user.id).type}`,
          value: "heavy",
        });
      }
      if (gagbondage) {
        outopts.push({
          name: `Gag: ${convertGagText(getGagLast(interaction.user.id))}`,
          value: "gag",
        });
      }
      if (mittenbondage) {
        outopts.push({
          name: `Mittens${mittenbondage.mittenname ? `: ${getMittenName(interaction.user.id)}` : ""}`,
          value: "mitten",
        });
      }
      if (chastitybondage) {
        outopts.push({
          name: `Chastity${chastitybondage.chastitytype ? `: ${getChastityName(interaction.user.id)}` : ""}`,
          value: "chastity",
        });
      }
      ...
      if (collarbondage) {
        outopts.push({
          name: `Collar${collarbondage.collartype ? `: ${getCollarName(interaction.user.id)}` : ""}`,
          value: "collar",
        });
      }
      ...
      if (outopts.length == 0) {
        ...
        outopts.push({
          name: "Struggle with nothing in particular...",
          value: "nothing",
        });
      }
