...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((opt) =>
      opt.setName("user").setDescription("Who to remove headwear from?"),
    )
    .addStringOption((opt) =>
      opt
        .setName("type")
  ...
  async autoComplete(interaction) {
    ...
    let chosenuserid =
      interaction.options.get("user")?.value ?? interaction.user.id; // Note we can only retrieve the user ID here!
    if (focusedValue == "") {
      ...
      try {
        ...
        // Remove anything we're already wearing from the list
        let sorted = process.headtypes.filter((f) =>
          itemsworn.includes(f.value),
        );
        sorted = sorted.filter((f) => !itemslocked.includes(f.value));
        let headstoreturn = sorted
          .filter((f) =>
  ...
  async execute(interaction) {
    try {
      let headwearuser = interaction.options.getUser("user")
      ...
      if (headwearchoice && data.textdata.c2 == undefined) {
        ...
        interaction.reply({
          content: `Something went wrong with your input. Please let Enraa know with the exact thing you put in the Type field!`,
            ...
            if (headwearchoice) {
              ...
              if (getHeadwear(headwearuser.id).includes(headwearchoice)) {
                ...
                if (
                  ...
                  interaction.reply({
                    content: getTextGeneric("unbind", datatogeneric),
                    flags: MessageFlags.Ephemeral,
                  });
                  let canRemove = await handleBondageRemoval(
                    interaction.user,
                    headwearuser,
                    "head restraints",
                  ).then(
                    async (res) => {
                      await interaction.editReply(
                        getTextGeneric("unbind_accept", datatogeneric),
                      );
                    ...
                    async (rej) => {
                      await interaction.editReply(
                        getTextGeneric("unbind_decline", datatogeneric),
                      );
                    },
                  );
                }
