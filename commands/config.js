  ...
  async interactionresponse(interaction) {
    try {
      ...
      if (optionparts[1] == "menuselector") {
        ...
        let optionschoice = configoptions[optionparts[2]][
          optionparts[3]
        ].choices.map((c) => c.value);
        let newindex =
          optionschoice.indexOf(
            getOption(interaction.user.id, optionparts[3]),
          ) + 1;
        ...
        if (
          typeof configoptions[optionparts[2]][optionparts[3]].choices[newindex]
            .select_function == "function"
        ) {
          configoptions[optionparts[2]][optionparts[3]].choices[
            newindex
          ].select_function(interaction.user.id);
        }

        // Finally, reprompt the user, now with the new choice set.
        interaction.update(
          await generateConfigModal(interaction, optionparts[2]),
        );
        ...
        let newindex =
          optionschoice.indexOf(
            getServerOption(interaction.guildId, optionparts[3]),
          ) + 1;
        if (newindex >= optionschoice.length) {
          newindex = 0;
        }
        ...
        if (
          ...
          configoptions[optionparts[2]][optionparts[3]].choices[
            newindex
          ].select_function(interaction, interaction.guildId);
        }
        ...
        if (typeof buttonpressed.customtext == "function") {
          data.desctext = data.desctext.replace(
            "CUSTOMTEXT",
            buttonpressed.customtext(interaction.user.id),
          );
        }
        ...
        await interaction.showModal(
          generateTextEntryModal(interaction, data, optionparts[3]),
        );
        ...
        let savedchannels = [];
        let failedtext = ``;
        let channelsselected = interaction.channels
          ? interaction.channels?.keys()
          : [];
        channelsselected = Array.from(channelsselected);
        let oldchannelsselected = getServerOption(
          interaction.guildId,
          "server-channelspermitted",
        );
        // Missing channels in channelsselected versus oldchannelsselected should be removed.
        let filteredlistfordeletes = oldchannelsselected.filter(
          (f) => !channelsselected.includes(f),
        );
        // delete them!
        ...
        let filteredlistforcreation = channelsselected.filter(
          (f) => !oldchannelsselected.includes(f),
        );
        ...
        let uniquesavedchannels = [
          ...new Set(savedchannels.concat(...channelsselected)),
        ];
        ...
        interaction.update(
          await generateConfigModal(
            ...
            optionparts[2],
            undefined,
            failedtext.length > 0 ? failedtext : undefined,
          ),
        );
  ...
  async modalexecute(interaction) {
    ...
    if (optionparts[3] == "dollvisorname") {
      ...
      await interaction.reply({
        content: `Updated your Doll Visor designation to ${choiceinput.slice(0, 30)}`,
        flags: MessageFlags.Ephemeral,
      });
      if (process.recentinteraction) {
        if (
          process.recentinteraction[interaction.user.id]?.timestamp + 895000 >
          performance.now()
        ) {
          await process.recentinteraction[
            interaction.user.id
          ].interaction.editReply(
            await generateConfigModal(
              process.recentinteraction[interaction.user.id].interaction,
              optionparts[2],
            ),
          );
        }
