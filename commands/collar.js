...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("collar")
    .setDescription(
      `Put a collar on, allowing others to /chastity, /heavy and /mitten you`,
    )
    .addUserOption((opt) =>
      opt.setName("keyholder").setDescription("Who can do anything to you?"),
    )
    .addBooleanOption((opt) =>
      opt.setName("freeuse").setDescription("Allow public access (Free Use?"),
    )
    .addStringOption((opt) =>
      opt
        .setName("type")
  ...
  async autoComplete(interaction) {
    ...
    if (focusedValue == "") {
      ...
      try {
        let collarstoreturn = optionstouse
          .filter((f) =>
  ...
  async execute(interaction) {
    try {
      ...
      if (
        freeuse &&
        getOption(interaction.user.id, "publicaccess") != "enabled"
      ) {
  ...
  async modalexecute(interaction) {
    try {
      ...
      let choice_mitten =
        interaction.fields.getStringSelectValues("mitten") == "mitten_yes"
          ? true
          : false;
      let choice_chastity =
        interaction.fields.getStringSelectValues("chastity") == "chastity_yes"
          ? true
          : false;
      let choice_heavy =
        interaction.fields.getStringSelectValues("heavy") == "heavy_yes"
          ? true
          : false;
      let choice_mask =
        interaction.fields.getStringSelectValues("mask") == "mask_yes"
          ? true
          : false;
      // lol consistency with naming scheme is hard
      let choice_collartype =
        interaction.customId.split("_")[3].length > 0
          ? `${interaction.customId.split("_")[3]}_${interaction.customId.split("_")[4]}`
          : undefined;
