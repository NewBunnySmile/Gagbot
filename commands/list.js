...
const restraints = {
  Heavy: heavytypes
    ...
    .map((heavy) => ({
      name: heavy.name,
      value: `Denial coefficient: ${heavy.denialCoefficient}`,
    })),
  Mitten: mittentypes
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((heavy) => ({ name: heavy.name, value: "" })),
  Gag: gagtypes
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((heavy) => ({ name: heavy.name, value: "" })),
  "Chastity Belt": chastitytypes
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((heavy) => ({
      name: heavy.name,
      value: `Denial coefficient: ${heavy.denialCoefficient}`,
    })),
  "Chastity Bra": chastitybratypes
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((heavy) => ({
      name: heavy.name,
      value: `Denial coefficient: ${heavy.denialCoefficient}`,
  ...
  Mask: headweartypes
    ...
    .map((heavy) => ({
      name: heavy.name,
      value:
        heavy.blockinspect || heavy.blockemote
          ? `Restricts: ${heavy.blockinspect ? `Inspect, ` : ``}${heavy.blockemote ? `Emote, ` : ``}`.slice(
              0,
              -2,
            )
          : ``,
  ...
  Collar: collartypes
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((heavy) => ({ name: heavy.name, value: "" })),
  Wearable: wearabletypes
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((heavy) => ({
      name: heavy.name,
      value: heavy.colorable ? `Colorable` : ``,
    })),
};
...
async function generateList(listchoice, page, details) {
  ...
  let maxpages = Math.ceil(
    restraints[listchoice.replace("+", " ")].length / PAGE_SIZE,
  );
  for (
    let i = (page - 1) * PAGE_SIZE;
    i <
    Math.min(page * PAGE_SIZE, restraints[listchoice.replace("+", " ")].length);
    i++
  ) {
    fulltext = `${fulltext}\n- **${restraints[listchoice.replace("+", " ")][i].name}**`;
    if (
      details &&
      restraints[listchoice.replace("+", " ")][i].value.length > 0
    ) {
      let backticked =
        "`" + restraints[listchoice.replace("+", " ")][i].value + "`";
  ...
  let buttons = [
    ...
    // Current Page
    new ButtonBuilder()
      .setCustomId(`list_none_${listchoice}_${page}_${details}`)
  ...
  Object.keys(restraints).forEach((k) => {
    let opt = new StringSelectMenuOptionBuilder()
      .setLabel(k)
      .setValue(`list_pageselect_${k.replace(" ", "+")}_${details}`);
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("list")
    .setDescription("Testing List stuff"),
    ...
    try {
      ...
      if (optionparts[1] == "menuselector") {
        ...
        interaction.update(
          await generateList(
            optionparts[2],
            parseInt(optionparts[3]) - 1,
            optionparts[4] == "true" ? true : false,
          ),
        );
