...
const {
  getCorset,
  removeCorset,
} = require("./../functions/corsetfunctions.js");
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((opt) =>
      opt.setName("user").setDescription("Who to remove the corset from"),
    ),
  async execute(interaction) {
    try {
      let corsetuser = interaction.options.getUser("user")
        ...
        if (corsetuser == interaction.user) {
          ...
          if (getCorset(corsetuser.id)) {
            ...
            if (getChastity(corsetuser.id)) {
              ...
              data.chastity = true;
              if (
                canAccessChastity(corsetuser.id, interaction.user.id).access
              ) {
          ...
          if (getCorset(corsetuser.id)) {
            ...
            if (getChastity(corsetuser.id)) {
              ...
              if (
                canAccessChastity(corsetuser.id, interaction.user.id).access &&
                !canAccessChastity(corsetuser.id, interaction.user.id).public
              ) {
