...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((opt) =>
      opt.setName("user").setDescription("The user to gag"),
    )
    .addStringOption((opt) =>
      opt
        .setName("gag")
  ...
  async execute(interaction) {
    try {
      ...
      let gagtype = interaction.options.getString("gag")
        ? interaction.options.getString("gag")
        : "ball";
      let gagintensity = interaction.options.getNumber("intensity")
      ...
      let oldgagname = gagtypes.find(
        (g) => g.value == getGagLast(gaggeduser.id),
      )?.name;
          ...
          if (getGag(gaggeduser.id)) {
            ...
            if (currentgag) {
              ...
              if (
                ...
                interaction.reply({
                  content: getTextGeneric("changebind", datatogeneric),
                  flags: MessageFlags.Ephemeral,
                });
                let canRemove = await handleBondageRemoval(
                  ...
                  async (res) => {
                    await interaction.editReply(
                      getTextGeneric("changebind_accept", datatogeneric),
                    );
                    await interaction.followUp(getText(data));
                    assignGag(
                      gaggeduser.id,
                  ...
                  async (rej) => {
                    await interaction.editReply(
                      getTextGeneric("changebind_decline", datatogeneric),
                    );
                  },
                );
              }
