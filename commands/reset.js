...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (
      ...
      if (
        getServerOption(interaction.guildId, "server-safewordroleid") === ""
      ) {
        // no safeword role was setup. Make the user talk to a mod.
        await interaction.reply({
          content:
            "Please DM a mod about this command if someone needs to be reset.",
          flags: MessageFlags.Ephemeral,
        });
      } else if (
        getServerOption(interaction.guildId, "server-safewordroleid") &&
        ...
        await interaction.reply({
          content:
            "Resetting all of your restraints because you are safeworded.",
          flags: MessageFlags.Ephemeral,
        });
