    ...
    try {
      ...
      let data = {
        ...
        textdata: {
          ...
          c2:
            (braorbelt == "chastitybelt"
              ? getChastityName(interaction.user.id, bondagetype)
              : getChastityBraName(interaction.user.id, bondagetype)) ??
            (braorbelt == "chastitybelt" ? "chastity belt" : "chastity bra"),
        },
      };
      ...
      if (braorbelt == "chastitybelt") {
        ...
        if (getHeavy(interaction.user.id)) {
          ...
          data.chastity = true;
          if (
            getChastity(interaction.user.id)?.keyholder == interaction.user.id
          ) {
          ...
          if (chastitykeyholder) {
            ...
            interaction.reply(
              `${interaction.user} puts a chastity belt on and clicks a tiny lock on it before stashing the key for safekeeping!`,
            );
        ...
        if (getHeavy(interaction.user.id)) {
          ...
          data.chastity = true;
          if (
            getChastityBra(interaction.user.id)?.keyholder ==
            interaction.user.id
          ) {
            // User tries to lock another belt on themselves and they have the key
            data.key_self = true;
          ...
          if (chastitykeyholder) {
            ...
            // Left it unlocked ---- This is currently an unused data path as there will ALWAYS be a keyholder.
            interaction.reply(
              `${interaction.user} puts a chastity bra on and clicks a tiny lock on it before stashing the key for safekeeping!`,
            );
