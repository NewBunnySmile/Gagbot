...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((opt) =>
      opt.setName("user").setDescription("Who to apply headwear to?"),
    )
    .addStringOption((opt) =>
      opt
        .setName("type")
  ...
  async autoComplete(interaction) {
    ...
    if (focusedValue == "") {
      ...
      let sorted = process.headtypes.filter(
        (f) => !itemsworn.includes(f.value),
      );
        ...
        if (getMitten(interaction.user.id)) {
          ...
          if (headwearuser.id == interaction.user.id) {
            ...
            if (getCollar(headwearuser.id)) {
              data.collar = true;
              if (
                getCollarPerm(headwearuser.id, "mask") &&
                canAccessCollar(headwearuser.id, interaction.user.id)
              ) {
                data.maskperm = true;
                if (getHeadwear(headwearuser.id).includes(headwearchoice)) {
                  // Wearing the headgear already, Ephemeral
