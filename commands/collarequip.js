...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("collarequip")
    .setDescription(
      `Put chastity, mittens or heavy bondage on someone with a collar`,
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((opt) =>
          opt.setName("user").setDescription("To who?").setRequired(true),
        )
        .addStringOption((opt) =>
          opt
            .setName("type")
  ...
  async autoComplete(interaction) {
    ...
    if (subc == "heavy") {
      if (focusedValue == "") {
        ...
        let heavies = process.heavytypes
          .filter((f) =>
      ...
      let beltorbra =
        interaction.options.get("braorbelt")?.value ?? "chastitybelt"; // Note we can only retrieve the user ID here!
      let optionstouse =
        beltorbra == "chastitybelt"
          ? chastitytypesoptions
          : chastitybratypesoptions;
      if (focusedValue == "") {
        // User hasn't entered anything, lets give them a suggested set of 10
        ...
        try {
          let chastitytoreturn = optionstouse
            .filter((f) =>
              f.name.toLowerCase().includes(focusedValue.toLowerCase()),
            )
            .slice(0, 10);
  ...
  async execute(interaction) {
    try {
      ...
      let keyholderuser = interaction.options.getUser("keyholder")
        ? interaction.options.getUser("keyholder")
        : interaction.user;
      let braorbelt =
        interaction.options.getString("braorbelt") ?? "chastitybelt";
      ...
      if (actiontotake == "heavy") {
        ...
        if (
          getBaseHeavy(bondagetype) &&
          getBaseHeavy(bondagetype).namefunction
        ) {
          data = await getBaseHeavy(bondagetype).namefunction(
            interaction,
            data,
          );
        } else {
        ...
        if (braorbelt == "chastitybelt") {
          ...
          data.textdata.c3 = getChastityBraName(
            interaction.user.id,
            bondagetype,
          );
        }
          ...
          if (actiontotake == "mittens") {
            ...
            if (getCollarPerm(collareduser.id, "heavy")) {
              if (getHeavy(collareduser.id)) {
                data.alreadyworn = true;
                interaction.reply({
                  content: `${collareduser} is already in bondage, wearing a ${getHeavy(collareduser.id).type}!`,
                  flags: MessageFlags.Ephemeral,
                });
              } else {
                data.allowed = true;
                await interaction.deferReply({ flags: MessageFlags.Ephemeral });
                await handleExtremeRestraint(
                  interaction.user,
                  collareduser,
                  "heavy",
                  bondagetype,
                  ...
                  async (success) => {
                    await interaction.followUp({
                      content: `Equipping ${convertheavy(bondagetype)} onto ${collareduser}!`,
                      withResponse: true,
                    });
