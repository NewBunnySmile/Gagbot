...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("vibe")
    .setDescription(
      "Add a vibrator/toy, causing stuttered speech and other effects",
    )
    .addUserOption((opt) =>
      opt.setName("user").setDescription("Who to add a fun vibrator to"),
    )
    .addStringOption((opt) =>
      opt
        .setName("type")
  ...
  async execute(interaction) {
    try {
      let vibeuser = interaction.options.getUser("user")
        ? interaction.options.getUser("user")
        : interaction.user;
      let vibeintensity = interaction.options.getNumber("intensity")
        ? interaction.options.getNumber("intensity")
        : 5;
      let vibetype = interaction.options.getString("type")
      ...
      if (getHeavy(interaction.user.id)) {
        ...
        if (vibeuser == interaction.user) {
          ...
          if (
            getVibe(vibeuser.id) &&
            (getVibe(vibeuser.id).some((vibe) => vibe.vibetype == vibetype) ||
              !vibetype)
          ) {
            ...
            if (getChastity(vibeuser.id)) {
              ...
              if (canAccessChastity(vibeuser.id, interaction.user.id).access) {
                ...
                data.key = true;
                const fumbleResults = rollKeyFumbleN(
                  interaction.user.id,
                  vibeuser.id,
                  2,
                );
                if (fumbleResults[0]) {
                  ...
                  if (config.getKeyLoss(vibeuser.id) && fumbleResults[1]) {
                    ...
                    if (vibetype) {
                      // specific single vibe
                      data.single = true;
                      let discardresult = discardChastityKey(
                        vibeuser.id,
                        interaction.user.id,
                      );
          ...
          if (
            ...
            if (getChastity(vibeuser.id)) {
              ...
              if (canAccessChastity(vibeuser.id, interaction.user.id).access) {
                ...
                getChastity(vibeuser.id)?.access === 0 &&
                vibeuser.id != interaction.user.id
              ) {
                // public access key
                data.public = true;
                if (vibetype) {
