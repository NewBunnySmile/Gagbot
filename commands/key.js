...
const {
  ...
  setOption,
  config,
} = require("./../functions/configfunctions.js");
const {
  getHeadwear,
  getHeadwearName,
  getLockedHeadgear,
  addLockedHeadgear,
  removeLockedHeadgear,
} = require("./../functions/headwearfunctions.js");
const {
  canAccessCollar,
  promptCloneCollarKey,
  cloneCollarKey,
  revokeCollarKey,
  getClonedCollarKeysOwned,
  getOtherKeysCollar,
  getCollar,
  transferCollarKey,
  promptTransferCollarKey,
  collartypes,
  getCollarName,
} = require("./../functions/collarfunctions.js");
const {
  canAccessChastity,
  promptCloneChastityKey,
  cloneChastityKey,
  revokeChastityKey,
  getClonedChastityKeysOwned,
  getOtherKeysChastity,
  getChastity,
  transferChastityKey,
  promptTransferChastityKey,
  chastitytypesoptions,
  chastitybratypesoptions,
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addUserOption((opt) =>
          opt.setName("newkeyholder").setDescription("Who to give the key to?"),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("swapitem")
        .setDescription(
          "Swap a worn restraint for another you have the key for...",
        )
        .addUserOption((opt) =>
          opt
            .setName("wearer")
    ...
    try {
      if (subcommand == "clone" || subcommand == "give") {
        ...
        let collarkeyholder =
          ...
          canAccessCollar(chosenuserid, interaction.user.id, undefined, true)
            .access;
        let chastitykeyholder =
          getChastity(chosenuserid) &&
          canAccessChastity(chosenuserid, interaction.user.id, undefined, true)
            .access;
        ...
        let ownedclonedchastitykeys = getClonedChastityKeysOwned(
          interaction.user.id,
        );
        let ownedclonedchastitybrakeys = getClonedChastityBraKeysOwned(
          interaction.user.id,
        );
        let ownedclonedcollarkeys = getClonedCollarKeysOwned(
          interaction.user.id,
        );

        let clonedchastitykeys = getOtherKeysChastity(interaction.user.id);
        let clonedchastitybrakeys = getOtherKeysChastityBra(
          interaction.user.id,
        );
        ...
        ownedclonedchastitykeys = ownedclonedchastitykeys.map((k) => {
          return {
            name: `Your key to ${interaction.guild.members.cache.get(k.split("_")[0])?.displayName}'s chastity belt`,
            value: `${k.split("_")[0]}_${interaction.user.id}_${k.split("_")[1]}`,
          };
        });
        ownedclonedchastitybrakeys = ownedclonedchastitybrakeys.map((k) => {
          return {
            name: `Your key to ${interaction.guild.members.cache.get(k.split("_")[0])?.displayName}'s chastity bra`,
            value: `${k.split("_")[0]}_${interaction.user.id}_${k.split("_")[1]}`,
          };
        });
        ownedclonedcollarkeys = ownedclonedcollarkeys.map((k) => {
          return {
            name: `Your key to ${interaction.guild.members.cache.get(k.split("_")[0])?.displayName}'s collar`,
            value: `${k.split("_")[0]}_${interaction.user.id}_${k.split("_")[1]}`,
          };
        });
        clonedchastitykeys = clonedchastitykeys.map((k) => {
          return {
            name: `${interaction.guild.members.cache.get(k.split("_")[1])?.displayName}'s key to ${interaction.guild.members.cache.get(k.split("_")[0])?.displayName}'s chastity belt`.slice(
              0,
              100,
            ),
            value: `${k}_chastitybelt`,
          };
        });
        clonedchastitybrakeys = clonedchastitybrakeys.map((k) => {
          return {
            name: `${interaction.guild.members.cache.get(k.split("_")[1])?.displayName}'s key to ${interaction.guild.members.cache.get(k.split("_")[0])?.displayName}'s chastity bra`.slice(
              0,
              100,
            ),
            value: `${k}_chastitybra`,
          };
        });
        clonedcollarkeys = clonedcollarkeys.map((k) => {
          return {
            name: `${interaction.guild.members.cache.get(k.split("_")[1])?.displayName}'s key to ${interaction.guild.members.cache.get(k.split("_")[0])?.displayName}'s collar`.slice(
              0,
              100,
            ),
            value: `${k}_collar`,
        ...
        let sorted = [
          ...clonedchastitykeys,
          ...clonedchastitybrakeys,
          ...clonedcollarkeys,
          ...ownedclonedchastitykeys,
          ...ownedclonedchastitybrakeys,
          ...ownedclonedcollarkeys,
        ];
        if (sorted.length == 0) {
          sorted = [
            { name: "No Eligible Keys To Revoke...", value: "nothing" },
          ];
        } else if (
          sorted
            .filter((f) =>
              f.name.toLowerCase().includes(focusedValue.toLowerCase()),
            )
            .slice(0, 25).length == 0 &&
        ...
        if (interaction.options.get("restraint")?.focused) {
          ...
          let chastitybrakeyholder =
            getChastityBra(chosenuserid) &&
            canAccessChastityBra(chosenuserid, interaction.user.id, true)
              .access;
  ...
  async execute(interaction) {
    try {
      ...
      if (subcommand == "clone") {
        let wearertoclone =
          interaction.options.getUser("wearer") ?? interaction.user;
        ...
        if (!wearertoclone || !chosenrestrainttoclone || !clonedkeyholder) {
          interaction.reply({
            content: `Something went wrong. The command was parsed as:\nClone ${wearertoclone}'s key for ${chosenrestrainttoclone} and give to ${clonedkeyholder}!`,
        ...
        if (
          chosenrestrainttoclone == "collar" &&
          getCollar(wearertoclone.id) &&
          canAccessCollar(
            wearertoclone.id,
            interaction.user.id,
            undefined,
        ...
        if (
          chosenrestrainttoclone == "chastitybelt" &&
          getChastity(wearertoclone.id) &&
          canAccessChastity(
            wearertoclone.id,
            interaction.user.id,
            undefined,
        ...
        if (
          chosenrestrainttoclone == "chastitybra" &&
          getChastityBra(wearertoclone.id) &&
          canAccessChastityBra(
            wearertoclone.id,
            interaction.user.id,
            undefined,
        ...
        if (!canclone) {
          interaction.reply({
            content: `You do not have the keys for ${wearertoclone}'s ${chosenrestrainttoclone}.`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        }
        ...
        if (interaction.user == clonedkeyholder) {
          interaction.reply({
            content: `You can't give yourself a copy of the primary key!`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        }
        ...
        try {
          ...
          if (confirmation.customId === "agreetoclonebutton") {
            ...
            if (
              wearertoclone == interaction.user ||
              wearertoclone == clonedkeyholder
            ) {
              let data = {
                textarray: "texts_key",
                textdata: {
              ...
              if (chosenrestrainttoclone == "collar") {
                await confirmation.update({
                  content: getTextGeneric("clone_accept_self", data.textdata),
                  components: [],
                });
              ...
              await confirmation.update({
                content: `Prompting the user for permission.`,
                components: [],
              });
              if (chosenrestrainttoclone == "collar") {
                let canRemove = await promptCloneCollarKey(
                  interaction.user,
                  wearertoclone,
                  clonedkeyholder,
                ).then(
                  async (res) => {
                    ...
                    await confirmation.editReply(
                      getTextGeneric("clone_accept", data.textdata),
                    );
                    await confirmation.followUp(getText(data));
                    cloneCollarKey(wearertoclone.id, clonedkeyholder.id);
                  },
                  async (rej) => {
                    ...
                    await interaction.editReply(
                      getTextGeneric("clone_decline", datatogeneric),
                    );
                  },
                );
              } else if (chosenrestrainttoclone == "chastitybelt") {
                let canRemove = await promptCloneChastityKey(
                  interaction.user,
                  wearertoclone,
                  clonedkeyholder,
                ).then(
                ...
                let canRemove = await promptCloneChastityBraKey(
                  interaction.user,
                  wearertoclone,
                  clonedkeyholder,
                ).then(
          ...
          await interaction.editReply({
            content:
              "Confirmation not received within 5 minutes, cancelling transfer.",
            components: [],
          });
          return;
        }
        ...
        if (!cloneresponse) {
          interaction.reply({
            content: `Something went wrong. You provided no option.`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        }

        let clonedkeyholder = await interaction.guild.members.fetch(
          cloneresponse.split("_")[1],
        );
        let wearer = await interaction.guild.members.fetch(
          cloneresponse.split("_")[0],
        );
        ...
        if (
          typeofrestraint == "collar" &&
          getCollar(wearer.id) &&
          canAccessCollar(wearer.id, interaction.user.id, undefined, true)
            .access
        ) {
          canrevoke = true;
        ...
        if (
          typeofrestraint == "chastitybelt" &&
          getChastity(wearer.id) &&
          canAccessChastity(wearer.id, interaction.user.id, undefined, true)
            .access
        ) {
          canrevoke = true;
        ...
        if (
          typeofrestraint == "chastitybra" &&
          getChastityBra(wearer.id) &&
          canAccessChastityBra(wearer.id, interaction.user.id, undefined, true)
            .access
        ) {
          canrevoke = true;
        ...
        if (
          typeofrestraint == "collar" &&
          getCollar(wearer.id) &&
          canAccessCollar(wearer.id, interaction.user.id).access &&
          clonedkeyholder == interaction.user
        ) {
          canrevoke = true;
          typeofrestraintreadable = "collar";
          choiceemoji = "<:collar:1449984183261986939>";
        ...
        if (
          typeofrestraint == "chastitybelt" &&
          getChastity(wearer.id) &&
          canAccessChastity(wearer.id, interaction.user.id).access &&
          clonedkeyholder == interaction.user
        ) {
          canrevoke = true;
          typeofrestraintreadable = "chastity belt";
        ...
        if (
          typeofrestraint == "chastitybra" &&
          getChastityBra(wearer.id) &&
          canAccessChastityBra(wearer.id, interaction.user.id).access &&
          clonedkeyholder == interaction.user
        ) {
          canrevoke = true;
          typeofrestraintreadable = "chastity bra";
        ...
        if (!canrevoke) {
          if (!isclone) {
            interaction.reply({
              content: `You do not have the primary keys for ${wearer}'s ${typeofrestraintreadable}.`,
              flags: MessageFlags.Ephemeral,
            });
            return;
          } else {
            interaction.reply({
              content: `You do not have a cloned key for ${wearer}'s ${typeofrestraintreadable}.`,
              flags: MessageFlags.Ephemeral,
            });
            return;
          }
        ...
        try {
          ...
          if (confirmation.customId === "agreetorevokebutton") {
            ...
            if (typeofrestraint == "collar") {
              await confirmation.update({
                content: getTextGeneric("revoke_accept", data.textdata),
                components: [],
              });
              await confirmation.followUp(getText(data));
        ...
        if (!wearer || !restraint || !newKeyholder) {
          interaction.reply({
            content: `Something went wrong. The command was parsed as:\nGive ${wearer}'s key for ${restraint} and give to ${newKeyholder}!`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        }
        ...
        if (interaction.user == newKeyholder) {
          interaction.reply({
            content: `You can't give yourself the key you're holding!`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        }
        ...
        if (!cangive) {
          interaction.reply({
            content: `You do not have the keys for ${wearer}'s ${restraint}.`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        }
        ...
        try {
          ...
          if (confirmation.customId === "agreetogivebutton") {
            ...
            if (
              wearer == interaction.user ||
              wearer == newKeyholder ||
              config.getKeyGivingAuto(wearer.id)
            ) {
              let data = {
                textarray: "texts_key",
                textdata: {
              ...
              if (restraint == "collar") {
                await confirmation.update({
                  content: getTextGeneric("give_accept_self", data.textdata),
                  components: [],
                });
                await confirmation.followUp(getText(data));
              ...
              if (restraint == "collar") {
                let canRemove = await promptTransferCollarKey(
                  interaction.user,
                  wearer,
                  newKeyholder,
                ).then(
                  async (res) => {
                    ...
                    data[restraint] = true;
                    await confirmation.editReply(
                      getTextGeneric("give_accept", data.textdata),
                    );
                    await confirmation.followUp(getText(data));
                    transferCollarKey(wearer.id, newKeyholder.id);
                  },
                  async (rej) => {
                    ...
                    await interaction.editReply(
                      getTextGeneric("give_decline", data.textdata),
                    );
                  },
                );
              } else if (restraint == "chastitybelt") {
                let canRemove = await promptTransferChastityKey(
                  interaction.user,
                  wearer,
                  newKeyholder,
                ).then(
                ...
                let canRemove = await promptTransferChastityBraKey(
                  interaction.user,
                  wearer,
                  newKeyholder,
                ).then(
        ...
        if (!wearer || !restrainttype || !newrestraint) {
          interaction.reply({
            content: `Something went wrong. The command was parsed as:\nSwap ${wearer}'s ${restrainttype} to a ${newrestraint}!`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        }
        ...
        if (restrainttype == "collar") {
          ...
          if (
            getCollar(wearer.id) &&
            canAccessCollar(wearer.id, interaction.user.id, true).access
          ) {
            permitted = true;
          }
          ...
          if (
            getChastity(wearer.id) &&
            canAccessChastity(wearer.id, interaction.user.id, true).access
          ) {
            permitted = true;
          }
          ...
          if (
            getChastityBra(wearer.id) &&
            canAccessChastityBra(wearer.id, interaction.user.id, true).access
          ) {
            permitted = true;
          }
        }
        ...
        if (!permitted) {
          interaction.reply({
            content: `You don't have access to unlock ${wearer}'s ${restrainttype}!`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        } else if (!newrestraintname) {
          interaction.reply({
            content: `Something went wrong with your new restraint selection!`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        }
        ...
        if (interaction.user.id == wearer.id) {
          ...
          if (restrainttype == "collar") {
            data.textdata.c1 =
              getCollarName(wearer.id, getCollar(wearer.id).collartype) ??
              "collar"; // Old collar
            ...
            data.textdata.c1 =
              getChastityName(wearer.id, getChastity(wearer.id).chastitytype) ??
              "chastity belt"; // Old collar
            data.textdata.c2 = newrestraintname;
            ...
            data.textdata.c1 =
              getChastityBraName(
                wearer.id,
                getChastityBra(wearer.id).chastitytype,
              ) ?? "chastity bra"; // Old collar
            data.textdata.c2 = newrestraintname;
