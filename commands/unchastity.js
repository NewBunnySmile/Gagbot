...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((opt) =>
      opt.setName("wearer").setDescription("Who to unlock..."),
    )
    .addStringOption((opt) =>
      opt
        .setName("braorbelt")
  ...
  async execute(interaction) {
    try {
      let chastitywearer = interaction.options.getUser("wearer")
        ? interaction.options.getUser("wearer")
        : interaction.user;
      let braorbelt =
        ...
        // Anyway, rewritten unchastity logic
          ...
          if (chastitywearer == interaction.user) {
            ...
            if (getChastity(chastitywearer.id)) {
              ...
              data.chastity = true;
              if (
                canAccessChastity(chastitywearer.id, interaction.user.id, true)
                  .access
              ) {
                ...
                if (fumbleResults[0]) {
                  ...
                  data.fumble = true;
                  if (
                    config.getKeyLoss(chastitywearer.id) &&
                    fumbleResults[1]
                  ) {
                    // We lost the key
                    data.discard = true;
        ...
        // Anyway, rewritten unchastity logic
          ...
          if (chastitywearer == interaction.user) {
              ...
              if (
                ...
                if (fumbleResults[0]) {
                  ...
                  if (
                    ...
                    // We lost the key
                    data.discard = true;
                    let discardresult = discardChastityBraKey(
                      chastitywearer.id,
                      interaction.user.id,
                    );
                    data[discardresult] = true;
            ...
            if (getChastityBra(chastitywearer.id)) {
              ...
              data.chastity = true;
              if (
                canAccessChastityBra(chastitywearer.id, interaction.user.id)
                  .access
              ) {
                // We have their chastity key
                data.key = true;
