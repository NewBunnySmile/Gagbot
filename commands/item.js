  ...
  async autoComplete(interaction) {
    ...
    try {
      if (subcommand == "protect") {
        ...
        let sortedwearable = itemswornwearable.filter(
          (f) => !itemslockedwearable.includes(f),
        );
        ...
        sortedwearable = sortedwearable.map((f) => {
          return {
            name: getWearableName(undefined, f),
            value: `${f}+wearable`,
          };
        });
    ...
    try {
      ...
      if (subcommand == "protect") {
        if (chosenitem && chosenitem != "nothing") {
          ...
          if (chosenitemparts[1] == "head") {
            if (getHeadwearName(undefined, chosenitemparts[0])) {
              ...
              interaction.reply({
                content: `Item ${replytextname} is an invalid item! Try again.`,
          ...
          interaction.reply({
            content: `Item ${replytextname} successfully protected!`,
            flags: MessageFlags.Ephemeral,
          });
        } else if (chosenitem == "nothing") {
          interaction.reply({
            content: `You chose nothing to protect.`,
            flags: MessageFlags.Ephemeral,
          });
        } else {
          interaction.reply({
            content: `Something went wrong choosing an item.`,
            flags: MessageFlags.Ephemeral,
          });
        }
      } else {
        if (chosenitem && chosenitem != "nothing") {
          ...
          interaction.reply({
            content: `Item ${replytextname} removed from protection!`,
            flags: MessageFlags.Ephemeral,
          });
        } else if (chosenitem == "nothing") {
          interaction.reply({
            content: `You chose nothing to unprotect.`,
            flags: MessageFlags.Ephemeral,
          });
        } else {
          interaction.reply({
