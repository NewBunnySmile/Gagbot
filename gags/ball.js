...
const garbleText = (text, intensity) => {
  ...
  for (let x = 0; x < words.length; x++) {
    ...
    for (const char of words[x]) {
      ...
      let newChar = ballGagCharMaps[Math.ceil(intensity / 2) - 1].get(
        char.toLowerCase(),
      );
      ...
      if (newChar) {
        ...
        if (
          newChar.length == 2 &&
          char.toLowerCase() == (prevChar ? prevChar.toLowerCase() : null)
        ) {
          //console.log("Prev: " + prevChar + "; Next: " + char)
          nextChar = isUppercase ? newChar[1].toUpperCase() : newChar[1];
        } else {
          nextChar = isUppercase
            ? newChar[0].toUpperCase() + (newChar[1] ? newChar[1] : "")
            : newChar;
        }
