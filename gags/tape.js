...
const garbleText = (text, intensity) => {
  ...
  for (let x = 0; x < textParts.length; x++) {
    let word = "";
    let allCaps =
      !textParts[x].match(/[a-z]/) && textParts[x].length > 1 ? true : false;
    ...
    for (let itr = 0; itr < textParts[x].length; itr++) {
      ...
      if (tapeCharMap.get(textParts[x][itr].toLowerCase())) {
        ...
        nextChar =
          isUppercase && ((itr == 0 && intensity < 9) || intensity < 7)
            ? tapeCharMap.get(textParts[x][itr].toLowerCase()).toUpperCase()
            : tapeCharMap.get(textParts[x][itr].toLowerCase());
