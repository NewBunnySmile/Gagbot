...
const handleConsent = async (interaction, user) => {
  ...
  try {
    ...
    await interaction.editReply({
      content: `Consent form agreed to by <@${testusertarget}>! Please re-run the command to tie!`,
      components: [],
    });
  } catch (err) {
    console.log(err);
    await interaction.editReply({
      content: `Consent form was not agreed to for <@${testusertarget}>! Please try to bind again to bring this form back up.`,
      components: [],
    });
  }
};
...
const collarPermModal = (interaction, keyholder, freeuse, collartype) => {
  const modal = new ModalBuilder()
    .setCustomId(`collar_${keyholder.id}_${freeuse ? "f" : "t"}_${collartype}`)
    .setTitle("Collar Permissions");
...
This restraint is intended to allow **others** to use /chastity, /mittens and /heavy on you!`;
  ...
  // Add labels to modal
  modal
    .addTextDisplayComponents(restrictionWarningText)
    .addLabelComponents(
      restrictionsLabelmitten,
      restrictionsLabelchastity,
      restrictionsLabelheavy,
      restrictionsLabelmask,
    );
...
const timelockChastityModal = (interaction, wearer) => {
  const modal = new ModalBuilder()
    .setCustomId(`timelock_${wearer.id}_chastitybelt`)
    .setTitle("Chastity Belt Timelock");
...
-# Once confirmed, you will have a final prompt before the timelock starts`;
  ...
  let accesswhileboundoptions = [
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Only the keyholder can access ${interaction.user.id == wearer.id ? "your" : `${wearer.displayName}'s`} belt`,
      )
      // Value returned to you in modal submission
      .setValue("access_kh"),
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Nobody, not even you, can access ${interaction.user.id == wearer.id ? "your" : `${wearer.displayName}'s`} belt`,
      )
      // Value returned to you in modal submission
      .setValue("access_no"),
  ];
  ...
  if (getOption(wearer.id, "publicaccess") != "disabled") {
    accesswhileboundoptions.unshift(
      new StringSelectMenuOptionBuilder()
        ...
        // Description of option
        .setDescription(
          `Everyone except ${interaction.user.id == wearer.id ? "you" : `${wearer.displayName}`} can vibe and corset ${interaction.user.id == wearer.id ? "you" : `${getPronouns(wearer.id, "object")}`}`,
        )
        // Value returned to you in modal submission
  ...
  let keyholderafteroptions = [
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Unlocks ${interaction.user.id == wearer.id ? "your" : `${wearer.displayName}'s`} belt, letting it fall off`,
      )
      // Value returned to you in modal submission
    ...
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Returns the keys to ${interaction.user.id == wearer.id ? "you" : `${wearer.displayName}`}`,
      )
      // Value returned to you in modal submission
...
const timelockChastityBraModal = (interaction, wearer) => {
  const modal = new ModalBuilder()
    .setCustomId(`timelock_${wearer.id}_chastitybra`)
    .setTitle("Chastity Bra Timelock");
...
-# Once confirmed, you will have a final prompt before the timelock starts`;
  ...
  let accesswhileboundoptions = [
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Only the keyholder can access ${interaction.user.id == wearer.id ? "your" : `${wearer.displayName}'s`} bra`,
      )
      // Value returned to you in modal submission
      .setValue("access_kh"),
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Nobody, not even you, can access ${interaction.user.id == wearer.id ? "your" : `${wearer.displayName}'s`} bra`,
      )
      // Value returned to you in modal submission
      .setValue("access_no"),
  ];
  ...
  if (getOption(wearer.id, "publicaccess") != "disabled") {
    accesswhileboundoptions.unshift(
      new StringSelectMenuOptionBuilder()
        ...
        // Description of option
        .setDescription(
          `Everyone except ${interaction.user.id == wearer.id ? "you" : `${wearer.displayName}`} can do things to ${interaction.user.id == wearer.id ? "you" : `${getPronouns(wearer.id, "object")}`}`,
        )
        // Value returned to you in modal submission
        .setValue("access_others"),
    );
  ...
  let keyholderafteroptions = [
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Unlocks ${interaction.user.id == wearer.id ? "your" : `${wearer.displayName}'s`} bra, letting it fall off`,
      )
      // Value returned to you in modal submission
      .setValue("keyholder_unlock"),
    new StringSelectMenuOptionBuilder()
...
const timelockCollarModal = (interaction, wearer) => {
  const modal = new ModalBuilder()
    .setCustomId(`timelock_${wearer.id}_collar`)
    .setTitle("Collar Timelock");
...
-# Once confirmed, you will have a final prompt before the timelock starts`;
  ...
  let accesswhileboundoptions = [
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Only the keyholder can access ${interaction.user.id == wearer.id ? "your" : `${wearer.displayName}'s`} collar`,
      )
      // Value returned to you in modal submission
      .setValue("access_kh"),
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Nobody, not even you, can access ${interaction.user.id == wearer.id ? "your" : `${wearer.displayName}'s`} collar`,
      )
      // Value returned to you in modal submission
      .setValue("access_no"),
  ];
  ...
  if (getOption(wearer.id, "publicaccess") != "disabled") {
    accesswhileboundoptions.unshift(
      new StringSelectMenuOptionBuilder()
        ...
        // Description of option
        .setDescription(
          `Everyone except ${interaction.user.id == wearer.id ? "you" : `${wearer.displayName}`} can collarequip ${interaction.user.id == wearer.id ? "you" : `${getPronouns(wearer.id, "object")}`}`,
        )
        // Value returned to you in modal submission
        .setValue("access_others"),
    );
  ...
  let keyholderafteroptions = [
    new StringSelectMenuOptionBuilder()
      ...
      // Description of option
      .setDescription(
        `Unlocks ${interaction.user.id == wearer.id ? "your" : `${wearer.displayName}'s`} collar, letting it fall off`,
      )
      // Value returned to you in modal submission
      .setValue("keyholder_unlock"),
    new StringSelectMenuOptionBuilder()
...
function timelockBuildConfirm(
  ...
  console.log([
    isNaN(unlockTime1) ? 0 : unlockTime1,
    isNaN(unlockTime2) ? 0 : unlockTime2,
  ]);
  console.log(
    [
      isNaN(unlockTime1) ? 0 : unlockTime1,
...
function checkBondageRemoval(userID, targetID, type) {
  ...
  console.log(
    useroption == "all_binder_and_keyholder" &&
      (canAccessChastity(targetID, userID, true).access ||
        canAccessCollar(targetID, userID, true).access),
  );
        ...
        .then((mess) => {
          ...
          collector.on("collect", async (i) => {
            ...
            if (i.customId == "acceptButton") {
              await mess.delete().then(() => {
                i.reply(
                  `Confirmed - ${user} is permitted to ${change ? `change your ${type}` : `take your ${type} off`}!`,
                );
              });
              res(true);
            } else {
              await mess.delete().then(() => {
                i.reply(
                  `Rejected - ${user} is blocked from ${change ? `changing your ${type}` : `taking your ${type} off`}!`,
                );
              });
              rej(true);
            }
          ...
          collector.on("end", async (collected) => {
            ...
            if (collected.length == 0) {
              await mess.delete().then(() => {
                i.reply(
                  `Timed out - ${user} is blocked from ${change ? `changing your ${type}` : `taking your ${type} off`}!`,
                );
              });
              rej(true);
            }
...
async function handleExtremeRestraint(user, target, type, restraint) {
  return new Promise(async (res, rej) => {
    ...
    if (
      !hasOption ||
      hasOption == "Enabled" ||
      (hasOption == "PromptOthers" && user.id == target.id)
    ) {
      res(true);
      return;
    ...
    switch (type) {
      case "heavy":
        ...
        break;
      case "gag":
        restraintfullname = process.gagtypes.find(
          (f) => f.value == restraint,
        )?.name;
        break;
      default:
    ...
    // We need to ASK
    let extrahelptext =
      configoptions["Extreme"][`extreme-${type}-${restraint}`]?.prompttext ??
      "Something went wrong retrieving this text.";
    ...
    await dmchannel
      ...
      .then((mess) => {
        ...
        collector.on("collect", async (i) => {
          ...
          if (i.customId == "acceptButton") {
            await mess.delete().then(() => {
              i.reply(
                `Confirmed - ${restraintfullname} will be equipped on you.`,
              );
            });
            res(true);
          } else {
            await mess.delete().then(() => {
              i.reply(
                `Rejected - ${restraintfullname} will NOT be equipped on you.`,
              );
            });
            rej(true);
          }
        ...
        collector.on("end", async (collected) => {
          ...
          if (collected.length == 0) {
            await mess.delete().then(() => {
              i.reply(
                `Timed out - ${restraintfullname} will NOT be equipped on you.`,
              );
            });
            rej(true);
          }
      ...
      .catch((err) => {
        console.log(
          `Error sending message for Extreme Restraint ${type} to ${target}.`,
        );
        rej("NoDM");
      });
  });
