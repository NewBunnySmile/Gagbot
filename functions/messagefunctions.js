...
const loadEmoji = async (client) => {
  ...
  let sortedupload = emojifileslocal.filter(
    (f) => !emojisbotfiltered.includes(f),
  ); // Sort out what needs to be uploaded
  sortedupload.forEach((s) => {
    client.application.emojis
      .create({
        attachment: path.resolve(emojifileslocalpath, `${s}.png`),
        name: s,
      })
      .then((emoji) => {
...
const messageSendImg = async (
  ...
  if (process.webhook[channel_id]) {
    ...
    attachs.forEach((f) => {
      attachments.push(
        new AttachmentBuilder(`./downloaded/${f.name}`, {
          name: f.name,
          spoiler: f.spoiler,
        }),
      );
...
const messageSendChannel = async (str, channel, components = []) => {
  try {
    ...
    if (channeltosendto) {
      if (
        channeltosendto.isSendable() &&
        !channeltosendto.archived &&
        !channeltosendto.locked
      ) {
        if (
          channeltosendto
            .permissionsFor(channeltosendto.guild.members.me)
          ...
          console.log(
            `Message ${str.slice(0, 30)}${str.length > 30 ? "..." : ""} sent to ${channeltosendto.name}`,
          );
        } else {
          // Warn!
          console.log(
            `Sending message to the parent channel since we don't have access to send to these threads!`,
          );
          let messageoutput = {
          ...
          console.log(
            `Message ${str.slice(0, 30)}${str.length > 30 ? "..." : ""} sent to ${channeltosendto.name}'s parent channel, ${channeltosendto.parent.name} due to no permissions.`,
          );
        }
      } else {
        // Warn!
        console.log(
          `Sending message to the parent channel since the thread isnt sendable!`,
        );
        let messageoutput = {
