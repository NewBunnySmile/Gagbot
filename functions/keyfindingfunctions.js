...
function getFumbleChance(keyholder, locked) {
  ...
  if (
    keyholder != locked &&
    (!config.getKeyFumblingOthers(keyholder) ||
      !config.getKeyFumblingOthers(locked))
  )
    return 0;
  ...
  if (chance < 100 && config.getBlessedLuck(keyholder))
    chance -= getUserVar(keyholder, "blessed") ?? 0;
...
async function handleKeyFinding(message) {
  ...
  if (
    Math.random() >
    (Math.min(message.content.length / 20, 20) * process.discardedKeys.length) /
      100
  )
    return;
  ...
  if (Math.random() < calcFindSuccessChance(message.author.id)) {
    ...
    try {
      ...
      if (
        findFunction(idx, message.author.id) &&
        wearerobjectinguild &&
        message.channel
          .permissionsFor(wearerobjectinguild)
  ...
  try {
    if (message.author.id == lockedUser)
      message.channel.send(
        `${message.author} has found the key to ${their(message.author.id)} ${restraint}!`,
      );
    else
      message.channel.send(
...
async function sendFindFumbleMessage(message, lockedUser, restraint) {
  try {
    if (message.author.id == lockedUser)
      message.channel.send(
        `${message.author} has found the key to ${their(message.author.id)} ${restraint} but fumbles when trying to pick it up!`,
      );
    else
      message.channel.send(
