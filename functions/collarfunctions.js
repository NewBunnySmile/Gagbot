...
const canAccessCollar = (collaruser, keyholder, unlock, cloning) => {
  ...
  if (unlock) {
    ...
    if (
      getCollar(collaruser)?.access != 2 &&
      getCollar(collaruser)?.keyholder == keyholder &&
      keyholder != collaruser
    ) {
      accessval.access = true;
    }
    // Allow unlocks by any keyholder if no timelock.
    if (
      getCollar(collaruser)?.access == undefined &&
      getCollar(collaruser)?.keyholder == keyholder
    ) {
      accessval.access = true;
    }
  ...
  if (cloning) {
    ...
    if (
      getCollar(collaruser)?.access == 1 &&
      getCollar(collaruser)?.keyholder == keyholder &&
      collaruser != keyholder
    ) {
      accessval.access = true;
    }
...
async function promptCloneCollarKey(user, target, clonekeyholder) {
  return new Promise(async (res, rej) => {
    ...
    let bondageaccess =
      `${getCollarPerm(target.id, "mitten") ? "mitten you, " : ""}${getCollarPerm(target.id, "chastity") ? "put you in chastity, " : ""}${getCollarPerm(target.id, "chastity") ? "put heavy bondage on you, " : ""}`.slice(
        0,
        -2,
      );
      ...
      .then((mess) => {
        ...
        collector.on("collect", async (i) => {
          ...
          if (i.customId == "acceptButton") {
            await mess.delete().then(() => {
              i.reply(
                `Confirmed - ${clonekeyholder} will receive a copied key for your collar!`,
              );
            });
            res(true);
          } else {
            await mess.delete().then(() => {
              i.reply(
                `Rejected - ${clonekeyholder} will NOT receive a copied key for your collar!`,
              );
            });
            rej(true);
          }
        ...
        collector.on("end", async (collected) => {
          ...
          if (collected.length == 0) {
            await mess.delete().then(() => {
              i.reply(
                `Timed Out - ${clonekeyholder} will NOT receive a copied key for your collar!`,
              );
            });
            rej(true);
          }
        ...
        .then((mess) => {
          ...
          collector.on("collect", async (i) => {
            ...
            if (i.customId == "acceptButton") {
              await mess.delete().then(() => {
                i.reply(
                  `Confirmed - ${newKeyholder} will receive the key for your collar!`,
                );
              });
              res(true);
            } else {
              await mess.delete().then(() => {
                i.reply(
                  `Rejected - ${newKeyholder} will NOT receive the key for your collar!`,
                );
              });
              rej(true);
            }
          ...
          collector.on("end", async (collected) => {
            ...
            if (collected.length == 0) {
              await mess.delete().then(() => {
                i.reply(
                  `Timed Out - ${newKeyholder} will NOT receive the key for your collar!`,
                );
              });
              rej(true);
            }
...
const revokeCollarKey = (collarUser, newKeyholder) => {
  ...
  if (collar.clonedKeyholders.includes(newKeyholder)) {
    collar.clonedKeyholders.splice(
      collar.clonedKeyholders.indexOf(newKeyholder),
      1,
    );
  }
  if (process.readytosave == undefined) {
