    ...
    collar: {
      key: {
        mitten: {
          ...
          nonamedmitten: {
            alreadyworn: [`TARGET_TAG is already wearing mittens!`],
            allowed: [
  ...
  noheavy: {
    chastity: {
      ...
      nokey: {
        ...
        other: [`You do not have the key for TARGET_TAG's chastity belt!`],
      },
    },
    nochastity: {
      self: {
...
const texts_heavy = {
  ...
  noheavy: [
    ...
    // Misc
    {
      only: (t) => {
        return (
          t.c2.includes("Festive Ribbons") || t.c2.includes("Wrapping Paper")
        );
      },
...
const texts_key = {
  ...
  give: {
    self: {
      collar: [`USER_TAG gives USER_THEIR collar key to VAR_C2.`],
      chastitybelt: [`USER_TAG gives USER_THEIR chastity belt key to VAR_C2.`],
      chastitybra: [`USER_TAG gives USER_THEIR chastity bra key to VAR_C2.`],
    },
    other: {
      collar: [
...
const texts_struggle = {
    ...
    noheavy: {
      ...
      nofingers: [
        ...
        `USER_TAG squirms in USER_THEIR VAR_C4, but no matter how much USER_THEY USER_TRY, USER_THEY just can't feel anything...`,
        {
          required: (t) => {
            return (
              getChastity(t.interactionuser.id).timestamp + 7200000 < Date.now()
            );
          },
  ...
  nostruggle: [
    ...
    // 2 hours in chastity
    {
      required: (t) => {
        return (
          !isNaN(getChastity(t.interactionuser.id)?.timestamp) &&
          getChastity(t.interactionuser.id)?.timestamp + 7200000 < Date.now()
        );
      },
    ...
    // 24 hours in chastity
    {
      required: (t) => {
        return (
          !isNaN(getChastity(t.interactionuser.id)?.timestamp) &&
          getChastity(t.interactionuser.id)?.timestamp + 86400000 < Date.now()
        );
      },
...
const texts_unheavy = {
  ...
  noheavy: {
    ...
    noheavyequipped: {
      self: [`You aren't in any kind of heavy bondage!`],
      other: [`TARGET_TAG is not in any kind of heavy bondage!`],
    },
  },
};
...
const texts_unvibe = {
  ...
  noheavy: {
    self: {
      ...
      novibe: {
        single: [`You do not have a VAR_C2 on yourself!`],
        both: [`You do not have any vibrators on yourself!`],
      },
    },
    other: {
      hasvibe: {
      ...
      novibe: {
        single: [`TARGET_TAG does not have a VAR_C2 on TARGET_THEM!`],
        both: [`TARGET_TAG does not have any vibrators on TARGET_THEM!`],
      },
    },
  },
};
      ...
      single: {
        worn: [
          ...
          },
          {
            only: (t) => {
              return (
                t.c2.includes("lasses") ||
                t.c2.includes("Librarian's Spectacles")
              );
            },
          ...
          },
          {
            only: (t) => {
              return (
                ...
                t.c2.includes("Boots") ||
                t.c2.includes("Pumps") ||
                t.c2.includes("Anklets") ||
                t.c2.includes("Greaves")
              );
            },
    ...
    self: {
      ...
      noworn: [
        ...
        },
        {
          only: (t) => {
            return (
              t.c2.includes("lasses") || t.c2.includes("Librarian's Spectacles")
            );
          },
    ...
    other: {
      ...
      noworn: [
        ...
        },
        {
          only: (t) => {
            return (
              t.c2.includes("Kissmark") &&
              getWearable(t.interactionuser.id).filter((f) =>
                f.includes("lipstick"),
              ).length > 0
            );
          },
...
const getTextGeneric = (type, data_in) => {
  let generics = {
    unbind:
      "TARGET_TAG has elected to prompt for TARGET_THEIR VAR_C1 to be removed. Please wait as TARGET_THEY confirmTARGET_S (5 minute timeout).",
    unbind_decline: "TARGET_TAG has declined your help with USER_THEIR VAR_C1.",
    unbind_accept:
      "TARGET_TAG has accepted your offer to help with TARGET_THEIR VAR_C1!",
    unbind_timeout: "The request to help TARGET_TAG timed out!",
    changebind:
      "TARGET_TAG has elected to prompt for TARGET_THEIR VAR_C1 to be changed. Please wait as TARGET_THEY confirmTARGET_S (5 minute timeout).",
    changebind_decline:
      "TARGET_TAG has declined allowing you to change TARGET_THEIR bindings.",
    changebind_accept:
      "TARGET_TAG has allowed you to change TARGET_THEIR bindings.",
    clone_accept:
      "TARGET_TAG has allowed you to make a clone of TARGET_THEIR VAR_C1 key, giving it to VAR_C2!",
    clone_accept_self: "Cloning your key...",
    clone_decline:
      "TARGET_TAG has forbidden you from making a clone of TARGET_THEIR VAR_C1 key for VAR_C2!",
    give_accept:
      "TARGET_TAG has allowed you to give TARGET_THEIR VAR_C1 key to VAR_C2!",
    give_accept_self: "Giving your key...",
    give_decline:
      "TARGET_TAG has forbidden you from giving TARGET_THEIR VAR_C1 key to VAR_C2!",
    revoke_accept:
      "You have destroyed the key VAR_C2 had to TARGET_TAG's VAR_C1.",
  };
  ...
  try {
    ...
    if (Array.isArray(sentencearr)) {
      ...
      if (only) {
        ...
        outstring =
          chosenphrases[Math.floor(Math.random() * chosenphrases.length)];
      }
      ...
      return (
        "There was an error generating this text. No error, but the destination was not an array of strings. Please tell Enraa that the tree followed this path: " +
        props.join(", ")
      );
    }
  } catch (err) {
