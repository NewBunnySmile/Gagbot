...
const {
  ...
  processHeadwearEmoji,
  getHeadwearName,
  getHeadwear,
  DOLLVISORS,
} = require("./headwearfunctions.js");
...
const DOLLREGEX =
  /(\s*\-#\s+)?(((?<![\*\\])\*{1})(\*{2})?(\\\*|[^\*]|\*.*\*|\*{2})+\*)(?!)|(((?<!\_)\_{1})(\_{2})?([^\_]|\_{2})+\_)|\n/g;

const DOLLPROTOCOL = [
  // Regex uses an ENQ character to not rematch matches.
  // Banned words
  {
    regex: /(?<![\u0005A-Za-z])i(?!((\.+i)|(-i)|['A-Za-z]))/i,
    value: 1,
    type: "1pp",
    string: "I",
  }, // "I"
  {
    regex: /(?<![\u0005A-Za-z])i'd(?![A-Za-z])/i,
    value: 1,
    type: "1pp",
    string: "I'd",
  }, // "I'd"
  {
    regex: /(?<![\u0005A-Za-z])i'm(?![A-Za-z])/i,
    value: 1,
    type: "1pp",
    string: "I'm",
  }, // "I'm"
  {
    regex: /(?<![\u0005A-Za-z])i'll(?![A-Za-z])/i,
    value: 1,
    type: "1pp",
    string: "I'll",
  }, // "I'll"
  {
    regex: /(?<![\u0005A-Za-z])i've(?![A-Za-z])/i,
    value: 1,
    type: "1pp",
    string: "I've",
  }, // "I've"
  {
    regex: /(?<![\u0005A-Za-z])my(?![A-Za-z])/i,
    value: 1,
    type: "1pp",
    string: "My",
  }, // "My"
  {
    regex: /(?<![\u0005A-Za-z])me(?![A-Za-z])/i,
    value: 1,
    type: "1pp",
    string: "Me",
  }, // "Me"
  {
    regex: /(?<![\u0005A-Za-z])myself(?![A-Za-z])/i,
    value: 1,
    type: "1pp",
    string: "Myself",
  }, // "Myself"
  ...
  {
    regex:
      /(c.{0,10}a.{0,10}t.{0,10}h.{0,10}e.{0,10}r.{0,10}i.{0,10}n.{0,10}e.{0,10}) ?w.{0,10}i.{0,10}l.{0,10}l.{0,10}o.{0,10}w.{0,10}s/gi,
    value: 999,
    type: "redact",
  }, // SHUT
];
...
async function textGarbleDOLL(msg, modifiedmessage, outtextin) {
  ...
  let dollProtocol = !(
    getOption(msg.author.id, "dollforcedprotocol") == "disabled"
  ); // Enabled for any level that isn't disabled
  ...
  if (dollified) {
    ...
    if (
      !dollIDOverride ||
      (Number.isFinite(dollIDOverride) && dollIDOverride.length < 6)
    ) {
      dollDigits = dollIDOverride
        ? dollIDOverride
        : `${msg.author.id}`.slice(-4);
      ...
      dollID =
        ...
        (dollDigits.length >= 4
          ? dollDigits
          : "0".repeat(4 - dollDigits.length) + dollDigits);
      dollIDColor = 34;
      // Display names max 32 chars.
      let truncateDisplay = "";
      try {
        truncateDisplay =
          msg.member.displayName.slice(0, 16) +
          (msg.member.displayName.length > 16 ? "..." : "");
      } catch (err) {
        console.error(err.message); // Following is not tested but SHOULD work.
        truncateDisplay =
          msg.author.displayName.slice(0, 16) +
          (msg.author.displayName.length > 16 ? "..." : "");
      }
      ...
      if (dollIDOverride.length < 25) {
        ...
        if (additionallength - 3 > msg.member.displayName.length) {
          ...
          let reducedname = msg.member.displayName.slice(
            0,
            Math.min(additionallength - 6, msg.member.displayName.length),
          );
    ...
    let partstolinkto = Array.from(
      outtext.matchAll(
        /(<(@|#)[0-9]+>)|(<?https?\:\/\/[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)>?)/g,
      ),
    ).map((a) => a[0]); // Match User tags, channel tags and links
    ...
    for (let i = 0; i < dollMessageParts.length; i++) {
      if (dollMessageParts[i].garble) {
        ...
        let replacebolds = Array.from(
          dollMessageParts[i].text.matchAll(
            /((\*\*)|(\_\_))[^(\*|\_)]+((\*\*)|(\_\_))/g,
          ),
        ).map((a) => a[0]);
        ...
        replacebolds.forEach((b) => {
          ...
          dollMessageParts[i].text = dollMessageParts[i].text.replace(
            b,
            replaceb,
          );
        });
        ...
        if (dollProtocol) {
          DOLLPROTOCOL.forEach((r) => {
            ...
            if (replaceProtocol) {
              dollProtocolVioType = dollProtocolVioType
                ? PROTOCOLVIOLATIONPRIOS[r.type] >
                  PROTOCOLVIOLATIONPRIOS[dollProtocolVioType]
                  ? r.type
                  : dollProtocolVioType
                : r.type;
              ...
              while (dollMessageParts[i].text.match(r.regex)) {
                if (
                  getOption(msg.author.id, "dollforcedprotocol") != "warning"
                ) {
                ...
                dollMessageParts[i].text = dollMessageParts[i].text.replace(
                  r.regex,
                  r.type == "redact"
                    ? `[1;40;30m[REDACTED][0m`
                    : `[0;31m[${dollMessageParts[i].text.match(r.regex)[0]}][0m`,
                );
              }
            }
          });
        ...
        if (
          (dollProtocolViolations > 0 || warnmodified) &&
          i == lastDollifiedMessage
        ) {
          let totalViolations = dollProtocolViolations;
          if (getOption(msg.author.id, "dollforcedprotocol") != "warning") {
            totalViolations =
              dollProtocolViolations + process.dolls[msg.author.id].violations;
          }

          // WARN if below punishment threshold. ERROR if exceeded.
          // CRITICAL if new violations >= punishmentThresh
          let violationTier =
            totalViolations >= getOption(msg.author.id, "dollpunishthresh")
              ? dollProtocolViolations >=
                Math.max(getOption(msg.author.id, "dollpunishthresh"), 2)
                ? "CRITICAL"
                : "ERROR"
              : "WARN";
          let violationColor =
            violationTier == "CRITICAL"
              ? "31m"
              : violationTier == "ERROR"
                ? "31m"
                : "33m";
          let violationcount =
            getOption(msg.author.id, "dollforcedprotocol") == "warning"
              ? ``
              : ` (${totalViolations}/${getOption(msg.author.id, "dollpunishthresh")})`; // Note, we do not need to check for "No" because the text won't show at all in that case.
          vioMessage =
            PROTOCOLVIOLATIONS[dollProtocolVioType][
              Math.floor(
                Math.random() * PROTOCOLVIOLATIONS[dollProtocolVioType].length,
              )
            ];
          dollMessageParts[i].text +=
  ...
  return {
    ...
    outtext: outtext,
    dollIDDisplay: dollIDDisplay,
    dollProtocolViolations: dollProtocolViolations,
  };
}
