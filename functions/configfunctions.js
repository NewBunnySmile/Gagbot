...
const configoptions = {
    ...
    removebondage: {
      ...
      choices: [
        ...
        },
        {
          ...
          helptext:
            "Prompt for anyone besides who put something on you or keyholders",
          select_function: (userID) => {
            return false;
          },
    ...
    publicaccess: {
      ...
      choices: [
        ...
        },
        {
          name: "Yes",
          helptext:
            "**⚠️ You can select public access options on collars and timelocks!**",
          select_function: (userID) => {
            return false;
          },
          value: "enabled",
  ...
  Misc: {
    dollvisorname: {
      ...
      descmodal:
        "What should your tag display as in Doll Visor? Your default Doll tag is CUSTOMTEXT.",
      choices: [
        {
          name: "Set Name",
    ...
    dollforcedprotocol: {
      ...
      choices: [
        ...
        },
        {
          name: "Yes",
          helptext:
            "Doll Visor will punish the wearer. This can apply mittens and heavy!",
          select_function: (userID) => {
            return false;
          },
          value: "enabled",
  ...
  return new Promise(async (res, rej) => {
    ...
    Object.keys(configoptions[menuset]).forEach(async (k) => {
        ...
        let buttonsection = new SectionBuilder()
          ...
          .setButtonAccessory((button) =>
            button
              ...
              .setLabel(
                configoptions[menuset][k].choices.find(
                  (f) => f.value == getOption(interaction.user.id, k),
                )?.name ?? "Undefined",
              )
              .setStyle(
                configoptions[menuset][k].choices.find(
              ...
              .setDisabled(
                configoptions[menuset][k].disabled(interaction.user.id),
              ),
          );
        ...
        let buttonsection = new SectionBuilder()
          ...
          .setButtonAccessory((button) =>
            button
              ...
              .setLabel(
                configoptions[menuset][k].choices[0].name ?? "Undefined",
              )
              .setStyle(
                configoptions[menuset][k].choices[0].style ??
                  ButtonStyle.Danger,
              )
              .setDisabled(
                configoptions[menuset][k].disabled(interaction.user.id),
      ...
      if (configoptions[menuset][k].menutype == "choice_dollcolor") {
        ...
        configoptions[menuset][k].menutype == "choice_server_refreshcmd"
      ) {
        if (process.configs.servers[interaction.guildId] != undefined) {
          let button = new ButtonBuilder()
            .setCustomId(`config_refreshcmdButton_${k}`)
            .setLabel(
              `Refresh Commands${getServerCmdRefresh(interaction.guildId) > 0 ? ` (Wait ${getServerCmdRefresh(interaction.guildId)}s)` : ""}`,
            )
          ...
          let buttonsection = new SectionBuilder()
            ...
            .setButtonAccessory((button) =>
              button
                ...
                .setLabel(
                  configoptions[menuset][k].choices.find(
                    (f) => f.value == getServerOption(interaction.guildId, k),
                  )?.name,
                )
                .setStyle(
                  configoptions[menuset][k].choices.find(
                ...
                .setDisabled(
                  configoptions[menuset][k].disabled(interaction.guildId),
                ),
            );
          pagecomponents.push(buttonsection);
        }
      } else if (
        configoptions[menuset][k].menutype == "choice_server_channels"
      ) {
        if (process.configs.servers[interaction.guildId] != undefined) {
          let currentrole = "Select allowed channels...";
          let channelsmentioned = [];
          if (
            getServerOption(interaction.guildId, "server-channelspermitted") &&
            getServerOption(interaction.guildId, "server-channelspermitted")
              .length > 0
          ) {
            channelsmentioned = getServerOption(
              interaction.guildId,
              "server-channelspermitted",
            );
          }

          let roledescription = new TextDisplayBuilder().setContent(
        ...
        if (process.configs.servers[interaction.guildId] != undefined) {
          ...
          let rolefetched;
          if (
            getServerOption(interaction.guildId, k) &&
            getServerOption(interaction.guildId, k).length > 0
          ) {
            rolefetched = await interaction.guild.roles.fetch(
              getServerOption(interaction.guildId, k),
            );
          }
          ...
          let disabled =
            getBotOption("bot-allownewsetup") == "Disabled" &&
            interaction.user.id != interaction.client.application.owner.id;
          let noserverdescription = new TextDisplayBuilder().setContent(
            `### This server does not yet have a configuration. Click the button below to setup default settings.\nSetting up **${interaction.guild.name}**`,
          );
          let button = new ButtonBuilder()
          ...
          let noserverdescription2 = new TextDisplayBuilder().setContent(
            disabled
              ? `-# The bot's owner has forbidden new installations except from them. Please contact them for initial setup.`
              : `-# You will then be able to use slash commands here.`,
          );
    ...
    if (
      ...
      allguilds.forEach(async (g) => {
        ...
        let guildsection = new SectionBuilder()
          .addTextDisplayComponents((textdisplay) =>
            textdisplay.setContent(
              `### ${guildappsset ? "Delete Config in " : "Create Default in "}${guildresolved.name}\n-# ‎   ⤷ ${guildappsset ? `Loaded with ${guildapps.length} commands` : `*Not Active on this Server*`}`,
            ),
          )
          .setButtonAccessory((button) =>
            button
              .setCustomId(
                `config_botguilds_${menuset}_${g}_${guildappsset ? "delete" : "setup"}`,
              )
              .setLabel(guildappsset ? "Delete Config" : "Setup Default Config")
              .setStyle(
                guildappsset ? ButtonStyle.Danger : ButtonStyle.Primary,
              ),
          );
    ...
    let menupageoptions = new StringSelectMenuBuilder().setCustomId(
      "config_menuselector",
    );
      ...
      optionspages.forEach((k) => {
        if (k == option) {
          if (typeof configoptions[p][k].default == "function") {
            process.configs.users[userID][k] =
              configoptions[p][k].default(userID);
          } else {
...
function getServerOption(serverID, option) {
  ...
  if (process.configs.servers[serverID][option] == undefined) {
    Object.keys(configoptions["Server"]).forEach((k) => {
      if (k == option) {
        process.configs.servers[serverID][k] =
          configoptions["Server"][k].default;
      }
    });
    if (process.readytosave == undefined) {
      process.readytosave = {};
...
async function removeAllCommands(interaction, serverID) {
  try {
    ...
    console.log(
      `Successfully discarded application (/) commands for server ID ${serverID}.`,
    );
  } catch (err) {
    console.log(err);
  }
...
function getServerCmdRefresh(serverID) {
  ...
  if (process.servercmdcooldown[serverID]) {
    console.log(
      process.servercmdcooldown[serverID].date - Math.floor(performance.now()),
    );
    return Math.floor(
      Math.max(
        Math.min(
          Math.floor(
            process.servercmdcooldown[serverID].date -
...
async function setCommands(interaction, serverID) {
  ...
  for (const file of commandFiles) {
    ...
    if (command.execute && command.data) {
      ...
      console.log(
        `Ignoring file at ./../commands/${file} because it does not have either a data or an execute export.`,
      );
    }
  }
  ...
  if (
    getServerOption(serverID, "server-allowapparel") == "Disabled" &&
    getServerOption(serverID, "server-allowhead") == "Disabled"
  ) {
  ...
  try {
    ...
    (async () => {
      console.log(
        `Trying to put ${commandsforrest.length} commands into ${serverID}`,
      );
      ...
      console.log(
        `Successfully reloaded ${data.length} application (/) commands into server ID ${serverID}.`,
      );
    })();
    ...
    process.servercmdcooldown[serverID] = {
      date: Math.floor(performance.now() + 60000) /* 1 Min cooldown */,
    };
    setTimeout(() => {
...
async function createWebhook(interaction, channel) {
  try {
    ...
    existingwebhooks.forEach((w) => {
      ...
      console.log(
        `ISBOT: ${w.applicationId != interaction.client.user.id}, ISNAME: ${w.name == "Gagbot"}`,
      );
...
function generateTextEntryModal(interaction, data, optionval) {
  ...
  const modal = new ModalBuilder()
    .setCustomId(`config_setoptionmodal_${data.page}_${optionval}`)
    .setTitle(`Enter Option...`);
  ...
  Object.entries(page).forEach(([key, option]) => {
    if (option.choices) {
      option.choices.forEach((choice) => {
        functions[`get${choice.uname}`] = (user) =>
          getOption(user, key) == choice.value;
      });
    }
  });
});
