...
const chastitytypes = [
  {
    name: "Featherlight Belt",
    value: "belt_featherlight",
    denialCoefficient: 15,
    minVibe: 2,
    minArousal: 1,
  },
  {
    name: "Blacksteel Chastity Belt",
    value: "belt_blacksteel",
    denialCoefficient: 7.5,
  },
  { name: "Silver Chastity Belt", value: "belt_silver", denialCoefficient: 5 },
  {
    name: "Ancient Chastity Belt",
    value: "belt_ancient",
    denialCoefficient: 15,
  },
  ...
  {
    name: "Chastity Belt of Eternal Denial",
    value: "belt_eternal",
    denialCoefficient: 2000,
  },
...
const chastitybratypes = [
  {
    name: "Featherlight Bra",
    value: "bra_featherlight",
    denialCoefficient: 6,
    minVibe: 2,
    minArousal: 1,
  },
  {
    name: "Blacksteel Chastity Bra",
    value: "bra_blacksteel",
    denialCoefficient: 3,
  },
...
const chastitytypesoptions = chastitytypes.map((chastity) => ({
  name: chastity.name,
  value: chastity.value,
}));
const chastitybratypesoptions = chastitybratypes.map((chastity) => ({
  name: chastity.name,
  value: chastity.value,
}));
...
const assignVibe = (user, intensity, vibetype = "bullet vibe", origbinder) => {
  ...
  if (!process.vibe[user]) {
    ...
    const existingVibe = process.vibe[user].find(
      (v) => v.vibetype === vibetype,
    );
    if (existingVibe) {
...
const removeVibe = (user, vibetype) => {
  ...
  if (!vibetype) {
    ...
    process.vibe[user] = process.vibe[user].filter(
      (v) => v.vibetype !== vibetype,
    );
...
const canAccessChastity = (chastityuser, keyholder, unlock, cloning) => {
  ...
  if (unlock) {
    ...
    if (
      getChastity(chastityuser)?.access != 2 &&
      getChastity(chastityuser)?.keyholder == keyholder &&
      keyholder != chastityuser
    ) {
      accessval.access = true;
    }
    // Allow unlocks by any keyholder if no timelock.
    if (
      getChastity(chastityuser)?.access == undefined &&
      getChastity(chastityuser)?.keyholder == keyholder
    ) {
      accessval.access = true;
    }
  ...
  if (cloning) {
    ...
    if (
      getChastity(chastityuser)?.access == 1 &&
      getChastity(chastityuser)?.keyholder == keyholder &&
...
const canAccessChastityBra = (chastityuser, keyholder, unlock, cloning) => {
  ...
  if (unlock) {
    // Allow unlocks by a non-self keyholder at all times, assuming its not sealed.
    if (
      getChastityBra(chastityuser)?.access != 2 &&
      getChastityBra(chastityuser)?.keyholder == keyholder &&
      keyholder != chastityuser
    ) {
      accessval.access = true;
    }
    // Allow unlocks by any keyholder if no timelock.
    if (
      getChastityBra(chastityuser)?.access == undefined &&
      getChastityBra(chastityuser)?.keyholder == keyholder
    ) {
      accessval.access = true;
    }
  ...
  if (cloning) {
    // Primary Keyholder access only if set to 0.
    if (
      getChastityBra(chastityuser)?.access == 0 &&
      keyholder != chastityuser
    ) {
      accessval.access = true;
      accessval.public = true;
    ...
    // Keyholder access if timelock is 1 (keyholder only) but only if not self.
    if (
      getChastityBra(chastityuser)?.access == 1 &&
      getChastityBra(chastityuser)?.keyholder == keyholder &&
      chastityuser != keyholder
    ) {
      accessval.access = true;
      ...
      .then((mess) => {
        ...
        collector.on("collect", async (i) => {
          ...
          if (i.customId == "acceptButton") {
            await mess.delete().then(() => {
              i.reply(
                `Confirmed - ${clonekeyholder} will receive a copied key for your chastity belt!`,
              );
            });
            res(true);
          } else {
            await mess.delete().then(() => {
              i.reply(
                `Rejected - ${clonekeyholder} will NOT receive a copied key for your chastity belt!`,
              );
            });
            rej(true);
          }
        ...
        collector.on("end", async (collected) => {
          ...
          if (collected.length == 0) {
            await mess.delete().then(() => {
              i.reply(
                `Timed Out - ${clonekeyholder} will NOT receive a copied key for your chastity belt!`,
              );
            });
            rej(true);
          }
        ...
        .then((mess) => {
          ...
          collector.on("collect", async (i) => {
            ...
            if (i.customId == "acceptButton") {
              await mess.delete().then(() => {
                i.reply(
                  `Confirmed - ${newKeyholder} will receive the key for your chastity belt!`,
                );
              });
              res(true);
            } else {
              await mess.delete().then(() => {
                i.reply(
                  `Rejected - ${newKeyholder} will NOT receive the key for your chastity belt!`,
                );
              });
              rej(true);
            }
          ...
          collector.on("end", async (collected) => {
            ...
            if (collected.length == 0) {
              await mess.delete().then(() => {
                i.reply(
                  `Timed Out - ${newKeyholder} will NOT receive the key for your chastity belt!`,
                );
              });
              rej(true);
            }
      ...
      .then((mess) => {
        ...
        collector.on("collect", async (i) => {
          ...
          if (i.customId == "acceptButton") {
            await mess.delete().then(() => {
              i.reply(
                `Confirmed - ${clonekeyholder} will receive a copied key for your chastity bra!`,
              );
            });
            res(true);
          } else {
            await mess.delete().then(() => {
              i.reply(
                `Rejected - ${clonekeyholder} will NOT receive a copied key for your chastity bra!`,
              );
            });
            rej(true);
          }
        ...
        collector.on("end", async (collected) => {
          ...
          if (collected.length == 0) {
            await mess.delete().then(() => {
              i.reply(
                `Timed Out - ${clonekeyholder} will NOT receive a copied key for your chastity bra!`,
              );
            });
            rej(true);
          }
        ...
        .then((mess) => {
          ...
          collector.on("collect", async (i) => {
            ...
            if (i.customId == "acceptButton") {
              await mess.delete().then(() => {
                i.reply(
                  `Confirmed - ${newKeyholder} will receive the key for your chastity bra!`,
                );
              });
              res(true);
            } else {
              await mess.delete().then(() => {
                i.reply(
                  `Rejected - ${newKeyholder} will NOT receive the key for your chastity bra!`,
                );
              });
              rej(true);
            }
          ...
          collector.on("end", async (collected) => {
            ...
            if (collected.length == 0) {
              await mess.delete().then(() => {
                i.reply(
                  `Timed Out - ${newKeyholder} will NOT receive the key for your chastity bra!`,
                );
              });
              rej(true);
            }
...
const revokeChastityKey = (chastityuser, newKeyholder) => {
  ...
  if (chastity.clonedKeyholders.includes(newKeyholder)) {
    chastity.clonedKeyholders.splice(
      chastity.clonedKeyholders.indexOf(newKeyholder),
      1,
    );
  }
  if (process.readytosave == undefined) {
...
const discardChastityKey = (user, locker) => {
  ...
  if (process.chastity[user]) {
    if (process.chastity[user].keyholder == locker) {
      ...
      process.chastity[user].clonedKeyholders.splice(
        process.chastity[user].clonedKeyholders.indexOf(locker),
        1,
      );
      typelocked = "clone";
    }
...
const discardChastityBraKey = (user, locker) => {
  ...
  if (process.chastitybra[user]) {
    if (process.chastitybra[user].keyholder == locker) {
      ...
      // Lost a clone. Clones will simply disappear.
      process.chastitybra[user].clonedKeyholders.splice(
        process.chastitybra[user].clonedKeyholders.indexOf(locker),
        1,
      );
      typelocked = "clone";
    }
  ...
  if (config.getDynamicArousal(user)) {
    ...
    for (const [min, max, minChange, maxChange, text] of arousedtexts) {
      if (
        (min < 0 || min <= current) &&
        (max < 0 || max >= current) &&
        (minChange < 0 || minChange <= change) &&
        (maxChange < 0 || maxChange >= change)
      )
        texts.push(text);
    }
  } else {
    ...
    for (const [min, max, _0, _1, text] of arousedtexts) {
      if ((min < 0 || min <= arousal) && (max < 0 || max >= arousal))
        texts.push(text);
    }
  }
...
function stutterText(msg, text, intensity, arousedtexts) {
  ...
  for (let i = 0; i < newtextparts.length; i++) {
    ...
    let prearousalchoicethresh = Math.min(
      (intensity / (40 * overcorrected) / 4) * usermod,
      0.25,
    );
    ...
    if (!modified && prearousalmathroll < prearousalcumulative && !nosyllable) {
      ...
      for (
        ...
        y <
        Math.min(
          Math.floor((Math.random() + 0.5) * stuttertimes),
          stuttertimes,
        );
        y++
      ) {
    ...
    let postarousalchoicethresh = Math.min(
      (intensity / (40 * overcorrected) / 3) * usermod * (modified ? 0.5 : 1.0),
      0.25,
    );
    ...
    if (
      !postmodified &&
      postarousalmathroll < postarousalcumulative &&
      !nosyllable
    ) {
      stuttered = true;
      postmodified = true;
    ...
    if (!postmodified && postarousalmathroll < postarousalcumulative) {
      ...
      postmodified = true;
      let arousedtext =
        arousedtexts[Math.floor(Math.random() * arousedtexts.length)] ??
        "mmf\\~";
...
function updateArousalValues() {
  try {
    ...
    for (const user in process.vibe)
      if (!process.arousal[user])
        process.arousal[user] = { arousal: 0, prev: 0, timestamp: now };
    for (const user in process.chastity)
      if (!process.arousal[user])
        process.arousal[user] = { arousal: 0, prev: 0, timestamp: now };
    for (const user in process.arousal) {
      ...
      if (getChastity(user)) {
        minArousal =
          chastitytypes.find((c) => c.value == chastity.chastitytype)
            ?.minArousal ?? 0;
      }
      if (getChastityBra(user)) {
        minArousal =
          minArousal +
          (chastitybratypes.find((c) => c.value == chastitybra.chastitytype)
            ?.minArousal ?? 0);
      }
...
function getVibeEquivalent(user) {
  ...
  if (intensity >= STUTTER_LIMIT) {
    ...
    if (chastity) {
      ...
      intensity +=
        (calcFrustration(hoursBelted) + (chastity.extraFrustration ?? 0)) / 20;
    }
  }
  return intensity;
}
...
function getArousalChangeDescription(user) {
  ...
  const lastChange =
    (arousal.arousal - arousal.prev) /
    (getBotOption("bot-timetickrate") / 60000);
...
function calcNextArousal(
  ...
  const noDecay =
    ...
    ((getBotOption("bot-timetickrate") / 60000) *
      (1 +
        AROUSAL_PERIOD_AMPLITUDE *
          Math.cos(time * AROUSAL_PERIOD_A) *
          Math.cos(time * AROUSAL_PERIOD_B)) *
      growthCoefficient *
      (RANDOM_BIAS + Math.random())) /
      (RANDOM_BIAS + 1);
  const next =
    noDecay -
    (getBotOption("bot-timetickrate") / 60000) *
      decayCoefficient *
      Math.max(arousal + prev / 2, 0.1);
  return next;
}
...
function tryOrgasm(user) {
  ...
  if (
    (arousal * (RANDOM_BIAS + Math.random())) / (RANDOM_BIAS + 1) >=
    orgasmLimit * denialCoefficient
  ) {
...
function calcGrowthCoefficient(user) {
  ...
  if (chastity)
    minVibe =
      chastitytypes.find((c) => c.value == chastity.chastitytype)?.minVibe ?? 0;
  if (chastitybra)
    minVibe =
      minVibe +
      (chastitybratypes.find((c) => c.value == chastitybra.chastitytype)
  ...
  return (
    Math.max(vibes?.reduce((a, b) => a + b.intensity, 0) ?? 0, minVibe) *
    VIBE_SCALING
  );
}
...
function calcDenialCoefficient(user) {
  ...
  const chastity = getChastity(user);
  if (chastity) {
    const denialCoefficient =
      chastitytypes.find((c) => c.value == chastity.chastitytype)
        ?.denialCoefficient ?? 5;
    return (
      (heavy ? heavyDenialCoefficient(heavy.typeval) : 0) / 2 +
      denialCoefficient
    );
  }
