...
const { messageSend, messageSendImg, messageSendChannel } = require(
  `./../functions/messagefunctions.js`,
);
const { getCorset, corsetLimitWords, silenceMessage } = require(
  `./../functions/corsetfunctions.js`,
);
const { stutterText, getArousedTexts } = require(
  `./../functions/vibefunctions.js`,
);
...
const { DOLLMAXPUNISHMENT, textGarbleDOLL } = require(
  `./../functions/dollfunctions.js`,
);
...
function punishDoll(userID, amount) {
  ...
  if (doll) {
    ...
    return {
      punish: punishments > 0 ? true : false,
      punishmentLevel: doll.punishmentLevel,
      skipped: skipped,
    };
  }
}
...
const modifymessage = async (msg, threadId) => {
  try {
    console.log(
      `${msg.channel.guild.name} - ${msg.member.displayName}: ${msg.content}`,
    );
    ...
    let corsetreturned = textGarbleCorset(
      messageparts,
      msg,
      modifiedmessage,
      threadId,
...
function replaceEmoji(msg, replacein, modifiedmessage) {
  ...
  if (!getHeadwearRestrictions(msg.author.id).canEmote) {
    replacingtext = processHeadwearEmoji(
      msg.author.id,
      msg.content,
...
function handleLinkExceptions(messagein) {
  ...
  for (let i = 0; i < messageparts.length - 1; i++) {
    ...
    if (
      current.text.startsWith("http://") ||
      current.text.startsWith("https://")
    ) {
...
async function sendTheMessage(
  ...
  try {
    ...
    if (msg.type == "19") {
      ...
      const replyauthorobject = await replied.guild.members.search({
        query: replied.author.displayName,
        limit: 1,
      });
    ...
    if (msg.attachments.size > 0) {
            ...
            https
              .get(attach[1].url, (response) => {
                ...
                file.on("finish", async () => {
                  file.close();
                  console.log(
                    `Downloaded file: ./downloaded/${attach[1].name}`,
                  );
      ...
      Promise.all(promisearr).then(async (v) => {
        ...
        messageSendImg(
          ...
          outtext,
          msg.member.displayAvatarURL(),
          dollIDDisplay ? dollIDDisplay : msg.member.displayName,
          threadID,
          attachments,
        ).then(() => {
    ...
    // No attachments to download
      ...
      if (!/[^\u0000-\u0020]/.test(outtext)) {
        ...
        outtext =
          "Miss <@125093095405518850>, I broke the bot! The bot said what I was trying to say, for debugging purposes. Unless it was 0 length somehow.";
      }
      // Check again, if we somehow got a 0 length text, something broke
      if (outtext.length == 0) {
        outtext =
          "Something went wrong. Ping <@125093095405518850> and let her know!";
      }
      // Finally send it!
      messageSend(
        msg,
