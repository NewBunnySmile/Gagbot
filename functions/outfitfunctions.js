...
function getOutfits(userID) {
    if (process.outfits == undefined) { process.outfits = {} }
    if (process.outfits[userID] == undefined) { process.outfits[userID] = [] };
...
async function generateOutfitModal(userID, menu, page, options) {
    ...
    if (menu == "restore") {
        ...
        let pagenavbuttons = [];
        pagenavbuttons.push(new ButtonBuilder()
    ...
    if (menu == "rename") {
        ...
        // Gag section
        let texts = `### Gags:\n`
        if (!getGag(userID)) { texts = `${texts}Not worn` }
        else {
            texts = `${texts}${getGags(userID).map((g) => convertGagText(g.gagtype)).join(", ")}`
        }
        pagecomponents.push(new SectionBuilder()
        ...
        // Headwear section
        texts = `### Headwear:\n`
        if (!(getHeadwear(userID).length > 0)) { texts = `${texts}Not worn` }
        else {
            texts = `${texts}${getHeadwear(userID).map((g) => getHeadwearName(undefined, g)).join(", ")}`
        }
        pagecomponents.push(new SectionBuilder()
            .addTextDisplayComponents((td) => td.setContent(texts))
            .setButtonAccessory((button) =>
        ...
        // Mittens section
        texts = `### Mitten:\n`
        if (!getMitten(userID)) { texts = `${texts}Not worn` }
        else {
        ...
        // Wearable section
        texts = `### Apparel:\n`
        if (!(getWearable(userID).length > 0)) { texts = `${texts}Not worn` }
        else {
            texts = `${texts}${getWearable(userID).map((w) => getWearableName(undefined,w)).join(", ")}`
        }
        pagecomponents.push(new SectionBuilder()
            .addTextDisplayComponents((td) => td.setContent(texts))
            .setButtonAccessory((button) =>
        ...
        // Vibrator section
        texts = `### Vibrators:\n`
        if (!getVibe(userID)) { texts = `${texts}Not worn` }
        else {
            texts = `${texts}${getVibe(userID).map((v) => v.vibetype).join(", ")}`
        }
        pagecomponents.push(new SectionBuilder()
            .addTextDisplayComponents((td) => td.setContent(texts))
            .setButtonAccessory((button) =>
        ...
        texts = `### Chastity Belt:\n`
        if (!getChastity(userID)) { texts = `${texts}Not worn` }
        else {
            ...
            texts = `${texts}${(getChastity(userID).clonedKeyholders) ? `, clones held by ${getChastity(userID).clonedKeyholders.map((k) => `<@${k}>`).join(", ")}` : ``}`
        }
        pagecomponents.push(new SectionBuilder()
            .addTextDisplayComponents((td) => td.setContent(texts))
            .setButtonAccessory((button) =>
        ...
        texts = `### Chastity Bra:\n`
        if (!getChastityBra(userID)) { texts = `${texts}Not worn` }
        else {
            ...
            texts = `${texts}${(getChastityBra(userID).clonedKeyholders) ? `, clones held by ${getChastityBra(userID).clonedKeyholders.map((k) => `<@${k}>`).join(", ")}` : ``}`
        }
        pagecomponents.push(new SectionBuilder()
            .addTextDisplayComponents((td) => td.setContent(texts))
            .setButtonAccessory((button) =>
        ...
        // Corset section
        texts = `### Corset:\n`
        if (!getCorset(userID)) { texts = `${texts}Not worn` }
        else {
        ...
        texts = `### Heavy Bondage:\n`
        if (!getHeavy(userID)) { texts = `${texts}Not worn` }
        else {
        ...
        // Collar section
        texts = `### Collar:\n`
        if (!getCollar(userID)) { texts = `${texts}Not worn` }
        else {
            ...
            texts = `${texts}${(getChastityBra(userID).clonedKeyholders) ? `, clones held by ${getCollar(userID).clonedKeyholders.map((k) => `<@${k}>`).join(", ")}` : ``}`
        }
        pagecomponents.push(new SectionBuilder()
            .addTextDisplayComponents((td) => td.setContent(texts))
            .setButtonAccessory((button) =>
...
function outfitEntryModal(interaction, slot) {
	...
	const choicetextentry = new TextInputBuilder()
		...
		.setPlaceholder("Enter outfit name...")
		.setRequired(true);
