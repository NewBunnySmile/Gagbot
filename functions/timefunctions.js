...
const {
  unlockTimelockChastity,
  unlockTimelockChastityBra,
  unlockTimelockCollar,
} = require(`./timelockfunctions.js`);
...
function importFileNames() {
  ...
  let eventfunctionsfolders = fs.readdirSync(
    path.resolve(__dirname, "..", "eventfunctions"),
  );
  eventfunctionsfolders.forEach((f) => {
    process.eventfunctions[f] = {};
    let eventfunctionsfiles = fs.readdirSync(
      path.resolve(__dirname, "..", "eventfunctions", f),
    );
    eventfunctionsfiles.forEach((file) => {
      let functionfile = require(
        path.resolve(__dirname, "..", "eventfunctions", f, file),
      );
      if (typeof functionfile.functiontick === "function") {
        process.eventfunctions[f][file.replace(".js", "")] =
          functionfile.functiontick;
      }
    });
  });
}
...
function runProcessedEvents() {
  ...
  if (process.gags) {
    Object.keys(process.gags).forEach((userid) => {
      getGags(userid).forEach((g) => {
        if (
          process.eventfunctions.gags &&
          process.eventfunctions.gags[g.gagtype]
        ) {
  ...
  if (process.headwear) {
    Object.keys(process.headwear).forEach((userid) => {
      getHeadwear(userid).forEach((h) => {
        if (
          process.eventfunctions.headwear &&
          process.eventfunctions.headwear[h]
        ) {
  ...
  if (process.mitten) {
    Object.keys(process.mitten).forEach((userid) => {
      if (getMitten(userid)) {
        if (
          process.eventfunctions.mitten &&
          process.eventfunctions.mitten[getMitten(userid).mittenname]
        ) {
  ...
  if (process.heavy) {
    Object.keys(process.heavy).forEach((userid) => {
      if (getHeavy(userid)) {
        if (
          process.eventfunctions.heavy &&
          process.eventfunctions.heavy[getHeavy(userid).typeval]
        ) {
  ...
  if (process.wearable) {
    Object.keys(process.wearable).forEach((userid) => {
      getWearable(userid).forEach((h) => {
        if (
          process.eventfunctions.wearable &&
          process.eventfunctions.wearable[h]
        ) {
